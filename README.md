...
| そのschemaの具体的な Graph 値を作りたい (個体集合をコンパイル時に固定する `static_graph_schema!` 向け) | `<schema名>!` (schema宣言が生成する、instance宣言用の `macro_rules!`) | 個体・辺を並べてGraph値を作る。利用者は、生成ファイルが持つ構築の入口 `{instance名}::construct::nodes!(..)` → `{instance名}::construct::edges!(&nodes)` → `{instance名}::Graph::new(&edges)` の順に Graph 値を組み立てる。値ありの個体・積み荷はinstance宣言の式からのみ供給され、これらのマクロ以外から差し替えることはできない。多重度・対一意の検査はコンパイル時に行う (詳細: `docs/static_graph.md`) |
