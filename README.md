...
- **宣言から通常の Rust ソースを生成します。** `cargo xtask generate` が書き出した
  ...
  マクロは生成ファイルとの指紋の一致だけをコンパイル時に検査します。
...
| やりたいこと | 使うもの | 何をするものか |
|---|---|---|
| ドメイン固有の型付きグラフ構造を宣言したい | `graph_schema!` | ノード種別・辺種別・役割名・制約を型付きで宣言する。実体は `cargo xtask generate` が書き出す通常の Rust ファイルで、マクロは指紋の照合だけを行う |
...
- **構築して凍結します。** `graph!` か `Builder` で組み立て、凍結時に全制約を検査
  します。最初の1件で止めず全違反を集めたい場合は `Graph::create_collecting` を
  使います。
- **完成後は `NodeRef` / `EdgeRef` から辿ります。** どちらも `&Graph` と内部位置だけを
  持つ軽い値です。種別全体への操作 (`graph.boss_iter()` ・ `graph.person_by_id(&id)`)
  の主語は `Graph`、関係の探索の主語は参照自身です。
...
`examples/` 配下に、`graphite` だけに依存するスタンドアロンクレートを7本置いています。
...
いません。** 根拠は次のとおりです。`cargo xtask` は `.cargo/config.toml` のエイリアス
