...
**一括構築 (`extend_nodes`/`extend_edges`)**: 実行時データ (合成データ生成器・
CSV 等) から構築する場合、要素単位の `insert`/`add` に加えてイテレータ版
`b.extend_nodes(iter of (key, value))`/`b.extend_edges(iter of (key, edge))`
も生成されます。戻り値はキー列 (`Vec<N::Id>`/`Vec<E::Id>`、挿入順)。意味論は
...
(`graph!` のノード項/辺項に書く識別子や、`insert`/`add`/`extend_nodes`/
`extend_edges` に渡すキー引数がそのまま `{Node}Id`/`{Kind}Id` の内部値になる)。
