//! 走査対象のディレクトリ木1つの所有と、その木からの Rust ファイルと
//! 生成ファイルの収集。
//!
//! このファイルは1ファイル100行の原則の例外である (区分: 再設計待ち)。この
//! ファイルは走査対象のディレクトリ木1つを所有し、Rust ファイルと生成ファイ
//! ルの収集をその木のメソッドとして持つ。このファイルは1つの型のメソッドの
//! 列であって統合した部分構造を持たないため、統合による超過の根拠が成り立た
//! ない。責務の量の判定と再設計は issue #28 のやること4 が行う。超過を許す
