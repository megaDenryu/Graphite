...
//!
//! 注意: このファイルは1ファイル100行の原則を満たせない箇所として明示的に許容
//! している。生成する凍結処理は「辺表の構築・重複キーの検出・両端点の実在検査・
//! 端点対の重複検査・2つの役割索引への積み込み・多重度検査」が1つの `for` ループを
//! 共有する1本の手続きであり、途中で切ると隣のファイルを読まないと意味が
//! 取れなくなる。単一の責務が自然に大きくなった例である。
