...
//!
//! このファイルは1ファイル100行の原則の例外である (区分: 統合による超過)。公開
//! ID型が既定生成と明示指定のどちらであるかを1つの判別共用体で持つ。本体は59行
//! で、残りは同居する単体テストである。超過を許す根拠の台帳は
...
//
// 利用者が書いたパスは schema module の内側から見えるパスへ構築時に1回だけ
// 読み替える。生成のたびに読み替えると、同じ意味の判断がトークン出力の回数だけ
// 繰り返されるためである。
// `スキーマが生成するID型` は schema module 内に `pub struct {型名}(pub String);`
// を生成する。`利用者が宣言した既存のID型` は利用者が別に宣言した型を参照し、その
// パスは生成 module 内から解決できる形に読み替え済みである。
pub enum 公開ID型 {
