...
//   宣言位置のローカル変数・引数・型引数を通常のRustの式と同じように
//   捕捉する。値を move する式では、束縛マクロを2回呼ぶとクロージャが
//   既に消費されているため2回目はコンパイルエラーになる (FnOnce、通常の
//   Rustの意味と一致する)。
...
// (`construct::nodes!`等) がinstanceの宣言位置と離れていても、クロージャの
...
mod tests {
    ...
    fn 関数内位置はlet束縛したクロージャと束縛マクロを生成する() {
        ...
        .to_string();
        assert!(コード.contains("let __graphite_captured_太郎_開発チーム = move ||"));
        assert!(コード.contains("macro_rules ! __graphite_bind_太郎_開発チーム"));
