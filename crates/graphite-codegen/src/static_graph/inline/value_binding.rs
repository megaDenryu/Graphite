...
//
// - 関数内 (`in fn`) の場合は `let` でクロージャを束縛する。束縛マクロが
//   生成するクロージャは `move` を付けない。付けると借りるだけの式 (`社員 { 名前:
...
// (`construct!`の展開結果である値マクロ) がinstanceの宣言位置と離れていても、
// クロージャの束縛名はmacro_rules!のローカル変数の衛生規則により宣言位置の
// 束縛を指す (呼び出し位置に同名のローカル・関数があってもすり替わらない。実測は
