...
// する。項目 (fn) の本体は常にその項目が書かれた位置の通常のスコープで
// 名前解決されるため、instanceがモジュール直下 (項目の位置) にあっても
// 関数の中 (文の位置) にあっても、`let`を使わずに宣言位置の意味を保てる
// (Rustは入れ子の項目をブロックの中にも置ける)。関数の中に置いた`fn`は
// 外側の関数のローカル変数・引数・型引数を捕捉できないため、値の式が
// それらを参照すると通常のRustのE0434になる (issue #46の再設計。旧設計は
// `in fn`の印でクロージャ束縛を選び分けていたが、オーナーの再レビューが
// ローカル捕捉というDSLの新機能自体を採らないと判断したため廃止した。
// 症状と対処は `docs/static_graph.md`「値の式の名前解決」節、compile-fail
// 実測は `crates/graphite/tests/ui/
// static_value_expr_referencing_local_is_rejected.rs`)。
//
// 束縛マクロ (`値束縛マクロ名`) の本体はローカル変数を持たず、宣言位置の
// 関数呼び出しの結果を返すだけであり、式そのものは束縛マクロの外へは
// 出ない。束縛マクロが呼ばれる位置 (`construct!`の展開結果である値マクロ)
// がinstanceの宣言位置と離れていても、関数名はmacro_rules!のローカル変数の
// 衛生規則により宣言位置の関数を指す (呼び出し位置に同名のローカル・関数が
// あってもすり替わらない。実測は
