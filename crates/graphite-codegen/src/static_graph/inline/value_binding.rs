...
// ラベルにしか効かないため、束縛マクロが呼ぶ関数名 (C分類の内部生成名
// `__graphite_value_*`) は`construct!`の呼び出し位置で解決される。利用者が
// この内部生成名と同じ名前の`fn`を呼び出し位置の近くで定義すると、束縛
// マクロの関数呼び出しがその`fn`に乗っ取られる余地があるが、この名前の
// 定義も直接呼び出しもGraphiteは支援しないため、この前提が保たれる。
//
