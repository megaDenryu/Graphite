// DSLトークンの型参照 (issue #41 §5.4)。instanceの展開へ、読むだけの型参照を
// 置く。種別トークン・辺名トークン・個体名トークン・端点トークンの4種類
// それぞれに、instanceに書かれたその出現箇所のspanを持つ型参照を置き、
// schema module越し・グラフmodule越しの修飾パス (`組織::所属Edge`・
// `開発チーム::{名前}Ref`) でF12の着地先を実際の型定義へ向ける。この型参照
// は実行時のコードを生まない (`const _: () = { .. };` の中の、呼ばれない
// 関数のシグネチャとしてだけ存在する)。
//
// 種別トークン・端点トークンは `具体辺` が持つ「instanceに書かれたトークン
// そのもの」を使う (解決済みの辺種別・個体の宣言位置を代用しない。
// 宣言箇所ではなくinstanceでの出現箇所そのもののspanを使う)。名前の組み立ては
// `naming::reference_paths`・`naming::internal_names` だけが行う
// (`format_ident!` はここに書かない)。

use proc_macro2::TokenStream;
use quote::quote;

use crate::static_graph::naming::{個体参照パス, 辺値参照パス, 辺参照パス, 型参照関数名};
use crate::static_graph::semantic::意味モデル;

pub(crate) fn dslトークンの型参照を組み立てる(意味モデル: &意味モデル) -> TokenStream {
    let 種別型参照列 = 意味モデル.具体辺列().iter().map(|辺| {
        let 型参照 = 辺値参照パス(意味モデル, 辺);
        quote! { _: &#型参照<'_> }
    });
    let 辺名型参照列 = 意味モデル.具体辺列().iter().map(|辺| {
        let 参照 = 辺参照パス(意味モデル, 辺.名前());
        quote! { _: &#参照<'_> }
    });
    let 端点型参照列 = 意味モデル.具体辺列().iter().flat_map(|辺| {
        辺.端点トークン列().into_iter().map(|トークン| {
            let 参照 = 個体参照パス(意味モデル, トークン);
            quote! { _: &#参照<'_> }
        })
    });
    let 個体型参照列 = 意味モデル.個体列().iter().map(|個体| {
        let 参照 = 個体参照パス(意味モデル, 個体.名前());
        quote! { _: &#参照<'_> }
    });
    let 引数列: Vec<TokenStream> =
        種別型参照列.chain(辺名型参照列).chain(端点型参照列).chain(個体型参照列).collect();
    let 関数名 = 型参照関数名();
    quote! {
        const _: () = {
            #[allow(dead_code)]
...
mod tests {
    ...
    #[test]
    fn 種別と個体それぞれの修飾済み型参照を持ち実行コードを生まない() {
        ...
        let トークン列 = dslトークンの型参照を組み立てる(&意味モデル);
        let コード = トークン列.to_string();
        assert!(コード.contains("fn __graphite_dsl_token_type_reference"));
        // 種別トークン: schema module越しに修飾する。
        ...
        assert!(関数.block.stmts.is_empty(), "型参照関数の本体は空でなければならない");
    }
    ...
    fn 自己ループ辺は端点トークンを2つ持つ() {
        ...
        let コード = dslトークンの型参照を組み立てる(&意味モデル).to_string();
