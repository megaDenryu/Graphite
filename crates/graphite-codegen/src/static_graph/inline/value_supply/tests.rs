...
const 生成パス文字列: &str = "generated/開発チーム.rs";
...
fn 個体値マクロは捕捉しない関数を経由し名前はグラフ名を含む() {
    ...
    let マクロ名 = 個体値マクロ名(意味モデル.グラフ名(), 生成先パス::new(生成パス文字列)).ident().to_string();
...
fn 同じグラフ名でも生成パスが違えば個体値マクロ名が衝突しない() {
    ...
    let 一つ目 = 個体値マクロ名(意味モデル.グラフ名(), 生成先パス::new("generated/m1.rs")).ident().to_string();
    let 二つ目 = 個体値マクロ名(意味モデル.グラフ名(), 生成先パス::new("generated/m2.rs")).ident().to_string();
...
fn 積み荷値マクロの名前はグラフ名を含みimplを使わない() {
    ...
    let マクロ名 = 積み荷値マクロ名(意味モデル.グラフ名(), 生成先パス::new(生成パス文字列)).ident().to_string();
    assert!(マクロ名.starts_with("__graphite_payloads_開発チーム_"), "グラフ名の後ろにinstance印が付くこと: {マクロ名}");

    let コード = 積み荷値マクロを組み立てる(&意味モデル, 生成先パス::new(生成パス文字列)).to_string();
    assert!(コード.contains(&マクロ名));
