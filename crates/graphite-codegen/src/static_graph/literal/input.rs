...
//
// instanceは、モジュール直下 (項目の位置) にも関数の中 (文の位置) にも
// 同じ `graph <名前>;` の1形式だけで書く。値の式は、どちらの位置でも
// 宣言位置に置いた捕捉しない `fn` の本体として固定する
// (`inline/value_binding.rs`)。`fn` は入れ子の項目としてブロックの中にも
// 置けるRustの通常の性質を使うため、instanceの構文自体は置かれた位置を
// 区別する必要が無い (以前あった `in fn` の印は issue #46 の再設計で廃止
// した。詳細は `docs/static_graph.md`「値の式の名前解決」節)。値の式が
// 関数のローカル変数・引数・型引数を参照すると、入れ子の `fn` は外側を
// 捕捉できないため、Rustの通常のE0434になる。
//
