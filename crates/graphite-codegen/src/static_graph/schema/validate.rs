...
impl 辺宣言 {
    ...
    // 端点の役割名と積み荷の役割名が同名だと、生成される辺値構造体の
    // フィールド名・{辺名}Refのアクセサ名が衝突する (issue #41 生成される
    // 名前の公開契約)。
    fn 端点役割と積み荷役割の衝突を検証する(&self) -> syn::Result<()> {
        let Some(積み荷) = self.形状.積み荷() else { return Ok(()) };
        if self.形状.端点役割一覧().iter().any(|端点役割| **端点役割 == 積み荷.役割) {
            return Err(syn::Error::new_spanned(
                &積み荷.役割,
                format!(
                    "名前の衝突: 辺 `{}` の積み荷役割 `{}` が端点の役割名と同じです。生成される辺値構造体のフィールドとアクセサが衝突します",
                    self.名前, 積み荷.役割
                ),
            ));
        }
