...
// ノード名・辺名の重複、辺の端点が未宣言のノードを指す誤り、同一辺内で
// 両端の役割名が同名になっている誤り (有向は始点役割/終点役割、無向は
// 第1役割/第2役割)、where節の役割名がその辺の宣言済み役割名と一致しない
// 誤りはここで検出する。
...
use super::input::{制約, 積み荷宣言, 辺形状, 辺宣言, 静的グラフ型入力};
    ...
    fn 辺の端点を検証する(&self, 辺宣言: &辺宣言) -> syn::Result<()> {
        match &辺宣言.形状 {
            辺形状::有向 { 始点役割, 始点型, 積み荷, 終点役割, 終点型 } => {
                ...
                self.端点役割と積み荷役割の衝突を検証する(辺宣言, &[始点役割, 終点役割], 積み荷)?;
            }
            辺形状::無向 { 第1役割, 第1型, 積み荷, 第2役割, 第2型 } => {
                self.型が宣言済みか検証する(第1型)?;
                self.型が宣言済みか検証する(第2型)?;
                if 第1役割 == 第2役割 {
                    return Err(syn::Error::new_spanned(
                        第2役割,
                        format!("辺 `{}` の第1役割と第2役割が同名です", 辺宣言.名前),
                    ));
                }
                self.端点役割と積み荷役割の衝突を検証する(辺宣言, &[第1役割, 第2役割], 積み荷)?;
            }
        }
        Ok(())
    }

    // 端点の役割名と積み荷の役割名が同名だと、生成される辺値structの
    // フィールド名・{辺名}Refのアクセサ名が衝突する (issue #41 生成される
    // 名前の公開契約、PR #45レビューE)。
    fn 端点役割と積み荷役割の衝突を検証する(
        &self,
        辺宣言: &辺宣言,
        端点役割達: &[&Ident],
        積み荷: &Option<積み荷宣言>,
    ) -> syn::Result<()> {
        let Some(積み荷) = 積み荷 else { return Ok(()) };
        if 端点役割達.iter().any(|端点役割| **端点役割 == 積み荷.役割) {
            return Err(syn::Error::new_spanned(
                &積み荷.役割,
                format!(
                    "名前の衝突: 辺 `{}` の積み荷役割 `{}` が端点の役割名と同じです。生成される辺値structのフィールド・アクセサが衝突します",
                    辺宣言.名前, 積み荷.役割
                ),
            ));
        }
