...
// てる。配線フィールドは `pub(super)` にし (issue #41 §5.3)、役割アクセサ・
// 積み荷アクセサは `pub` + 意味カードにする。役割・端点の対応は
// `具体辺形状` から直接読み、schemaの辺形状と突き合わせ直さない (issue #41
// 是正15)。辺値型の参照は `{schema名}::{種別}Edge` に修飾する (issue #41
// 是正2)。
...
use crate::static_graph::naming::{
    個体参照型名, 役割アクセサの追跡情報を作る, 辺値参照パス, 辺参照型名, 積み荷アクセサの追跡情報を作る,
};
...
pub(super) fn 辺インスタンス参照列を組み立てる(意味モデル: &意味モデル, 宣言元: &宣言元ファイルの綴り) -> TokenStream {
    let 生成列 = 意味モデル.具体辺列().iter().map(|辺| 一辺分を組み立てる(意味モデル, 辺, 宣言元));
    quote! { #(#生成列)* }
}

fn 一辺分を組み立てる(意味モデル: &意味モデル, 辺: &具体辺, 宣言元: &宣言元ファイルの綴り) -> TokenStream {
...
fn 役割アクセサ列を組み立てる(意味モデル: &意味モデル, 辺: &具体辺, 宣言元: &宣言元ファイルの綴り) -> TokenStream {
...
fn 積み荷アクセサを組み立てる(辺: &具体辺, 宣言元: &宣言元ファイルの綴り) -> TokenStream {
    match 辺.種別().積み荷() {
        Some(積み荷宣言 { 役割, 型 }) => {
