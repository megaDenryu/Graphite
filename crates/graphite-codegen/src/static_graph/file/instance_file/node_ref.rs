...
// `pub(super)` にし、docは付けない (issue #41 §5.3)。`entity()` と所属辺
// メソッドは `pub` + 意味カードにする。
...
use crate::static_graph::naming::{個体参照型名, 実体アクセサメソッド名, 辺アクセサメソッドの追跡情報を作る, 辺参照型名};
use crate::static_graph::semantic::{個体, 意味モデル};

use super::super::doc_render::doc属性を組み立てる;

pub(super) fn 個体参照列を組み立てる(意味モデル: &意味モデル, 宣言元: &宣言元ファイルの綴り) -> TokenStream {
    let 生成列 = 意味モデル.個体列().iter().map(|個体| 一個体分を組み立てる(意味モデル, 個体, 宣言元));
    quote! { #(#生成列)* }
}

fn 一個体分を組み立てる(意味モデル: &意味モデル, 個体: &個体, 宣言元: &宣言元ファイルの綴り) -> TokenStream {
    let 実体型 = 個体.実体型();
