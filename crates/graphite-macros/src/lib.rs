...
/// (instance展開がimplを一切使わず、値の橋渡しを呼び出し位置の`macro_rules!`だけで
/// 行うため)。
...
/// 生成ファイルが持つ構築の入口 `{グラフ名}::construct::nodes!(..)` へ実行時に
/// 渡す)。
///
/// 生成される `macro_rules!` はschema宣言と同じテキスト順の制約を持つ:
/// `static_graph_schema! { schema <名前> { .. } }` より後ろの行でしか
/// `<名前>! { .. }` を呼べない。利用者は、生成ファイルが持つ構築の入口
/// (`{グラフ名}::construct::nodes!`・`{グラフ名}::construct::edges!`) でグラフ本体の
/// 材料を組み立て、`DevTeam::Graph::new(&edges)` のようにinstance moduleへの
/// 修飾パスでグラフ本体を構築する。値ありの個体・積み荷は、instance宣言の式から
/// これらのマクロを介してのみ供給され、他の経路から差し替えることはできない。
