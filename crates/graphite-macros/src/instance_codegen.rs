...
//! の検証は凍結時なので意味論は変わらない。スプライスの (0..*) 系の挿入順保証
//! には第2段内の記述順がそのまま現れる、`docs/graph_splice.md` §1)。
//!
...
//! ファイルは150行を超える。このファイルは `graph!` のコード生成本体である。
//! 責務の量の判定は issue #28 のやること4 が行う。超過を許す根拠の台帳は
