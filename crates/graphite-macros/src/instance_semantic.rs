...
pub fn 構文を検証してグラフリテラルを組み立てる(
    ...
    Ok(検証済みグラフリテラル {
        ...
        ノード項の列: 並べ替え済み項.ノード項の列,
        残り項の列: 並べ替え済み項.残り項の列,
    })
}

/// [`端点を検証して全ノードと残りの2段へ並べ替える`] の戻り値。ノード項の列と
/// 残り項の列のどちらがどちらかを型で区別するため、無名タプルではなく
...
fn 端点を検証して全ノードと残りの2段へ並べ替える(
    ...
    let mut 残り項の列: Vec<検証済み残り項> = Vec::new();
    ...
    for item in items {
        match item {
            ...
            GraphItem::Edge(edge) => {
                ...
                残り項の列.push(検証済み残り項::辺(edge));
            }
            GraphItem::Spread(spread) => {
                残り項の列.push(検証済み残り項::スプライス(spread));
            }
        }
    }
