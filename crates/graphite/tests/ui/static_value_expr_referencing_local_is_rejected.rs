...
// compile-fail回帰試験。instance宣言は、モジュール直下にあっても関数の中に
// あっても、常に宣言位置に置いた捕捉しない`fn`の本体として値の式を固定する
// (`docs/static_graph.md`「値の式の名前解決」節)。入れ子の`fn`は外側の関数の
// ローカル変数・引数を捕捉できないため、値の式がそれらを参照すると通常の
// RustのE0434になる (外側のジェネリックの型引数を参照した場合のE0401は
// `static_value_expr_referencing_outer_type_param_is_rejected.rs`が別に
// 固定する)。対処は、その個体を値なし宣言 (`node 名前: 型;`) にして、実体を
