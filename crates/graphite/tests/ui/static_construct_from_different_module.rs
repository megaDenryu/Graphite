...
// (1) `construct::nodes!`自体は`pub`なmoduleの中の`pub(crate) use`された
// マクロなので別moduleからも呼び出せるが、その展開の内部で無修飾のまま
// 参照する値マクロ (`__graphite_values_{グラフ名}!`) の名前は呼び出し位置
// (`mod 別`) を起点に解決されるため、instance宣言を持つ`mod 元`の外からは
// 見えず解決に失敗する。(2) 値マクロ自身を`元::__graphite_values_開発チーム!`
// のように修飾パスで直接呼ぶ迂回も検査する。値マクロは意図的に
// `pub(crate) use`を持たない
...
mod 別 {
    pub fn 試す() {
        let _nodes = super::元::開発チーム::construct::nodes!();
