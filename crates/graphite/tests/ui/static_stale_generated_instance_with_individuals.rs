...
// エラーの並びをそのまま固定する。
//
// PR #45より前はE0080の後ろにE0061 (`Nodes::new` の引数の個数不一致) が
// もう1件続いていた (instance展開が呼び出し位置に生成する組み立て関数が
// `Nodes::new` を直接呼んでいたため)。PR #45で構築の入口を
// `construct::nodes!`/`construct::edges!` へ移し、instance宣言だけでは
// 内部構築子を呼ばなくなったため、このテストのようにinstance宣言だけを
// 書いて構築を呼ばない場面ではE0061は出なくなった。再生成を促す結論
// (E0080) が最後に出ることに変わりはない。
