...
//! 回帰試験 (issue #46、PR #45で名前解決を宣言位置へ固定する形に作り直した)。
//! `graph <名前> in fn;` の `in fn` が、値の式をlet束縛したクロージャで
//! 固定する形を選ぶ印である (`docs/static_graph.md`「値の式の名前解決」節)。
//! クロージャは宣言位置のローカル変数・引数・型引数を通常のRustの式と
//! 同じように捕捉する。
...
// 入れ子の`fn`は外側の引数を捕捉できないが、`in fn`のクロージャは
// 宣言位置の引数を通常のRustのクロージャと同じように捕捉する。
#[allow(non_snake_case, dead_code, private_interfaces)]
#[allow(clippy::needless_lifetimes, clippy::wrong_self_convention, clippy::clone_on_copy, clippy::write_literal)]
mod 検証チーム引数 {
