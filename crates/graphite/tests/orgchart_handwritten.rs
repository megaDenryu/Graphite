//! これは `dynamic_graph_schema!` が生成する想定のコードを手で書いたもの。
//! マクロ実装のテンプレート。
//!
...
//! `dynamic_graph_schema!` を実装するときに「展開後の目標コード」として突き合わせる
//! 対象になる。
//!
...
//! 変換が必要になり、かえって `dynamic_graph_schema!` の展開規則が複雑になる。
//!
//! それよりも「1 スキーマ宣言 = 1 独立 struct、1 エッジ種別 = 1
//! `HashMap` フィールド」という対応の方が、`dynamic_graph_schema!` マクロが
...
//! ファイルは150行を超える。このファイルはマクロが生成すべきコードの手書き
//! テンプレートである。扱いの判定は issue #28 のやること4 が行う。超過を許
