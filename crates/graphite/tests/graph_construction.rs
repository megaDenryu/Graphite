...
//! ファイルは検証対象1つ (汎用Graphの構築経路と構築時の失敗) に対するテスト
//! 関数の列である。このファイルはスキーマを `mod common` へ外へ出しているた
//! め、このファイルの分割は宣言を複製しない。テスト関数は互いに独立している。
//! 統合による超過の根拠が成り立たないため、テストの分け方の判定は issue #28
//! のやること4 が行う。超過を許す根拠の台帳は
