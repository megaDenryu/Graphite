...
//!
//! 端点のキーと辺のキーは、生成ID型か利用者が宣言したID型かで表示できるかが
//! 別々に決まる。このファイルは、4つの位置 (有向辺の始点・終点、無向辺の端点、
//! `unique pair` の端点対) をID型の組み合わせで試験できる schema を1つ宣言する。
//! 位置ごとの試験は `unknown_endpoint_positions` モジュールが持つ。各位置で、
//! 表示できる綴りだけが載り、省いた綴りには「表示できない」が添うことを固定する
//! (issue #26)。
...
graphite::graph_schema! {
    ...
    schema 診断 {
        ...
        edge 両端が生成キーの専有交友 = 生成キーの地点 -- 生成キーの地点 where unique pair;
        edge 両端が宣言キーの専有親交 = 宣言キーの地点 -- 宣言キーの地点 where unique pair;
    }
}
