...
//! ファイルは検証対象1つ (名前付き構築) に対するテスト用スキーマとテスト関
//! 数の列を持つ。`each_declaration_order.rs` が `#[path]` で宣言を親に残し
//! たままテストを部分モジュールへ出す技法を実証したため、このファイルの分割
//! が同じ宣言を各ファイルへ複製するという統合の根拠は成り立たない。検証観点
//! ごとに部分モジュールへ分ける判定を issue #28 のやること4 で行う。超過を
