...
// 生成元: tests/static_value_expr_name_resolves_at_call_site.rs:51
...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    12480351837679236554u64, 16142031460028305123u64, 16627813602015377348u64,
    12590116413933739144u64,
];
...
pub mod construct {
    ...
    ///
    /// 関係する instance 宣言: `tests/static_value_expr_name_resolves_at_call_site.rs` の `graph 名前解決チームトップレベル`
    macro_rules! nodes {
        () => {
            { let (太郎, 開発部,) =
            __graphite_values_名前解決チームトップレベル!();
            #[allow(deprecated)] let __graphite_nodes =
            名前解決チームトップレベル::Nodes::__graphite_internal_new(太郎,
    ...
    macro_rules! edges {
        ($nodes:expr) => {
            { let () = __graphite_payloads_名前解決チームトップレベル!();
            #[allow(deprecated)] let __graphite_edges =
            名前解決チームトップレベル::Edges::__graphite_internal_new($nodes,);
            __graphite_edges }
        };
    }
    pub(crate) use edges;
