...
// 生成元: tests/graph_clone_not_required.rs:28
...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    5964580237612893116u64, 17615011338967444755u64, 3649046090661027870u64,
    9743486325496711066u64,
];
/// `人物` ノードの公開ID。
///
/// 宣言: `tests/graph_clone_not_required.rs` の `node 人物`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct 人物Id(pub String);
/// `商品` ノードの公開ID。
///
/// 宣言: `tests/graph_clone_not_required.rs` の `node 商品`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct 商品Id(pub String);
/// `購入` 辺の公開ID。
///
/// 宣言: `tests/graph_clone_not_required.rs` の `edge 購入 = (購入者: 人物) -[取引: 取引情報]-> (対象商品: 商品) where unique pair`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct 購入Id(pub String);
/// `担当` 辺の公開ID。
///
/// 宣言: `tests/graph_clone_not_required.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct 担当Id(pub String);
/// `友人` 辺の公開ID。
///
/// 宣言: `tests/graph_clone_not_required.rs` の `edge 友人 = 人物 -- 人物 where unique pair`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
...
/// 凍結時の図式適合検査が見つけた違反。
///
/// 宣言: `tests/graph_clone_not_required.rs` の `schema 複製しない世界`
...
pub trait 複製しない世界Insertable: Sized {
...
pub trait 複製しない世界DefaultId: 複製しない世界Insertable {
    #[doc(hidden)]
...
/// 直接呼ぶことは想定しない。
pub trait 複製しない世界Node: 複製しない世界Insertable {}
impl 複製しない世界Insertable for super::人物 {
    type Id = 人物Id;
...
impl 複製しない世界DefaultId for super::人物 {
    fn insert_named_with_binding(
        self,
        b: &mut Builder,
        binding: String,
        ...
        複製しない世界Insertable::insert_named_with_id(
            self,
            b,
            人物Id(binding),
            permit,
    ...
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製しない世界Insertable::insert_with_id(self, b, 人物Id(binding))
    }
}
impl 複製しない世界Node for super::人物 {}
/// 完成済みグラフ上の `人物` ノード個体。
///
/// 宣言: `tests/graph_clone_not_required.rs` の `node 人物`
#[derive(Clone, Copy)]
...
impl 複製しない世界Insertable for super::商品 {
    type Id = 商品Id;
...
impl 複製しない世界DefaultId for super::商品 {
    fn insert_named_with_binding(
        self,
        b: &mut Builder,
        binding: String,
    ...
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製しない世界Insertable::insert_with_id(self, b, 商品Id(binding))
    }
}
impl 複製しない世界Node for super::商品 {}
/// 完成済みグラフ上の `商品` ノード個体。
///
/// 宣言: `tests/graph_clone_not_required.rs` の `node 商品`
#[derive(Clone, Copy)]
...
/// (`{Builder}::add` 経由で使う)。
pub trait 複製しない世界Edge: 複製しない世界Insertable {}
impl 複製しない世界Insertable for 購入 {
    type Id = 購入Id;
...
impl 複製しない世界DefaultId for 購入 {
    ...
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製しない世界Insertable::insert_with_id(self, b, 購入Id(binding))
    }
}
impl 複製しない世界Edge for 購入 {}
impl 複製しない世界Insertable for 担当 {
    type Id = 担当Id;
...
impl 複製しない世界DefaultId for 担当 {
    ...
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製しない世界Insertable::insert_with_id(self, b, 担当Id(binding))
    }
}
impl 複製しない世界Edge for 担当 {}
impl 複製しない世界Insertable for 友人 {
    type Id = 友人Id;
...
impl 複製しない世界DefaultId for 友人 {
    ...
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製しない世界Insertable::insert_with_id(self, b, 友人Id(binding))
    }
}
impl 複製しない世界Edge for 友人 {}
impl Builder {
    fn new() -> Self {
        Self {
    ...
    where
        N: 複製しない世界Node + 複製しない世界DefaultId,
    {
    ...
    /// `graph!` を経由しない。
    pub fn insert_with_id<N: 複製しない世界Node>(
        &mut self,
        id: N::Id,
        value: N,
    ) -> N::Id {
    ...
    /// (許可証は通常の `create` 経路からの直接的・偶発的な誤用を防ぐためのものであり、名前付き位置の持ち出しの検出は構築印の照合が担う。`crates/graphite/src/schema_runtime/named_construction.rs` 参照)。
    #[doc(hidden)]
    pub fn insert_named_with_id<N: 複製しない世界Node>(
        &mut self,
        id: N::Id,
        value: N,
        permit: &graphite::NamedInsertPermit,
    ...
    where
        E: 複製しない世界Edge + 複製しない世界DefaultId,
    {
        value.insert_with_binding(self, key.into())
    }
    ...
    /// `graph!` を経由しない。
    pub fn add_with_id<E: 複製しない世界Edge>(
        &mut self,
        id: E::Id,
        value: E,
    ) -> E::Id {
    ...
    /// (許可証は通常の `create` 経路からの直接的・偶発的な誤用を防ぐためのものであり、名前付き位置の持ち出しの検出は構築印の照合が担う。`crates/graphite/src/schema_runtime/named_construction.rs` 参照)。
    #[doc(hidden)]
    pub fn add_named_with_id<E: 複製しない世界Edge>(
        &mut self,
        id: E::Id,
        value: E,
        permit: &graphite::NamedInsertPermit,
