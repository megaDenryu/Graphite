...
// 生成元: tests/static_value_expr_not_duplicated_into_generated_file.rs:39
...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    13821333907874144160u64, 3069689336657365651u64, 14016283402600944646u64,
    3946020846464327202u64,
];
...
/// - 実体型: `社員`
///
/// 宣言: `tests/static_value_expr_not_duplicated_into_generated_file.rs` の `node 太郎: 社員 = ..`
#[derive(Clone, Copy)]
pub struct 太郎Ref<'a> {
    graph: &'a Graph,
}
impl<'a> 太郎Ref<'a> {
    ...
    /// - 戻り値: `太郎の所属Ref`
    ///
    /// 宣言: `tests/static_value_expr_not_duplicated_into_generated_file.rs` の `edge 太郎の所属 = 所属(太郎 -> 開発部)`
    ///
...
/// - 実体型: `部署`
///
/// 宣言: `tests/static_value_expr_not_duplicated_into_generated_file.rs` の `node 開発部: 部署 = ..`
#[derive(Clone, Copy)]
pub struct 開発部Ref<'a> {
    graph: &'a Graph,
}
...
impl<'a> 太郎の所属Ref<'a> {
    ...
    ///
    /// 関係する instance 宣言: `tests/static_value_expr_not_duplicated_into_generated_file.rs` の `edge 太郎の所属 = 所属(太郎 -> 開発部)`
...
///
/// 関係する instance 宣言: `tests/static_value_expr_not_duplicated_into_generated_file.rs` の `graph 複製検査チーム`
macro_rules! construct {
    () => {
        { let (太郎, 開発部,) =
        __graphite_values_複製検査チーム_4b442018df85ce6d!(); let () =
        __graphite_payloads_複製検査チーム_4b442018df85ce6d!();
        #[allow(deprecated)] {
        複製検査チーム::Graph::__graphite_internal_new(太郎, 開発部) } }
    };
}
