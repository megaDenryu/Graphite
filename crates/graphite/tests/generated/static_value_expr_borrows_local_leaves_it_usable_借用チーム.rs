...
// 生成元: tests/static_value_expr_borrows_local_leaves_it_usable.rs:41
...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    477807965003376659u64, 13794040824138464360u64, 12625129100803331909u64,
    6144941905061364097u64,
];
...
/// - 実体型: `社員`
///
/// 宣言: `tests/static_value_expr_borrows_local_leaves_it_usable.rs` の `node 太郎: 社員 = ..`
#[derive(Clone, Copy)]
pub struct 太郎Ref<'a> {
    graph: &'a Graph,
}
impl<'a> 太郎Ref<'a> {
    ...
    /// - 戻り値: `太郎の所属Ref`
    ///
    /// 宣言: `tests/static_value_expr_borrows_local_leaves_it_usable.rs` の `edge 太郎の所属 = 所属(太郎 -> 開発部)`
    ///
    /// 関係する schema 宣言: `tests/static_value_expr_borrows_local_leaves_it_usable.rs` の `edge 所属 = (member: 社員) -> (team: 部署) where each member: 1`
...
/// - 実体型: `部署`
///
/// 宣言: `tests/static_value_expr_borrows_local_leaves_it_usable.rs` の `node 開発部: 部署 = ..`
#[derive(Clone, Copy)]
pub struct 開発部Ref<'a> {
    graph: &'a Graph,
}
...
impl<'a> 太郎の所属Ref<'a> {
    ...
    ///
    /// 関係する instance 宣言: `tests/static_value_expr_borrows_local_leaves_it_usable.rs` の `edge 太郎の所属 = 所属(太郎 -> 開発部)`
...
///
/// 関係する instance 宣言: `tests/static_value_expr_borrows_local_leaves_it_usable.rs` の `graph 借用チーム`
macro_rules! construct {
    () => {
        { let (太郎, 開発部,) =
        __graphite_values_借用チーム_a3a281eff82b211a!(); let () =
        __graphite_payloads_借用チーム_a3a281eff82b211a!(); #[allow(deprecated)] {
        借用チーム::Graph::__graphite_internal_new(太郎, 開発部) } }
    };
}
