...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    8373870462102682598u64, 4980448372119223101u64, 15322325569388079628u64,
    9655481858646602088u64,
];
/// `人物` ノードの公開ID。
///
/// 宣言: `tests/graph_clone.rs` の `node 人物`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct 人物Id(pub String);
/// `商品` ノードの公開ID。
///
/// 宣言: `tests/graph_clone.rs` の `node 商品`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct 商品Id(pub String);
/// `購入` 辺の公開ID。
///
/// 宣言: `tests/graph_clone.rs` の `edge 購入 = (購入者: 人物) -[取引: 取引情報]-> (対象商品: 商品) where unique pair`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct 購入Id(pub String);
/// `担当` 辺の公開ID。
///
/// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct 担当Id(pub String);
/// `友人` 辺の公開ID。
///
/// 宣言: `tests/graph_clone.rs` の `edge 友人 = 人物 -- 人物 where unique pair`
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
struct __担当InternalPosition(graphite::TablePosition);
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
#[doc(hidden)]
#[derive(Clone, Copy)]
pub struct __担当NamedPosition(__担当InternalPosition, u64);
#[doc(hidden)]
#[derive(Clone, Copy)]
...
impl graphite::DirectedEdgeLiteral<人物Id, 商品Id, ()> for 担当 {
    fn from_graph_literal(from: 人物Id, to: 商品Id, (): ()) -> Self {
        Self::new(from, to)
    }
}
impl std::fmt::Debug for 担当 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple(stringify!(担当))
            .field(&self.担当者)
            .field(&self.担当商品)
            .finish()
    }
...
#[derive(Clone)]
struct __担当Record {
    担当者: __人物InternalPosition,
    担当商品: __商品InternalPosition,
}
#[allow(dead_code)]
#[derive(Clone)]
struct __友人Record {
...
impl std::fmt::Display for Violation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Violation::担当DuplicateKey(id) => {
                write!(f, "{}のキーが重複しています: {:?}", "担当", id)
            }
            Violation::担当UnknownSource { edge, source } => {
                write!(
                    f,
                    "未知のキー {:?} が {} として見つかりません (辺 `{}` {:?} の{})",
                    source, "人物", "担当", edge, "始点"
                )
            }
            Violation::担当UnknownTarget { edge, target } => {
                write!(
                    f,
                    "未知のキー {:?} が {} として見つかりません (辺 `{}` {:?} の{})",
                    target, "商品", "担当", edge, "終点"
                )
            }
            Violation::担当担当商品EachViolation { target, count } => {
                write!(
                    f,
                    "多重度制約違反: 辺 `{}` は {} {:?} について入次数 {} を期待しますが実際は {} 本です",
                    "担当", "商品", target, "ちょうど1", count
                )
            }
            Violation::担当担当者EachViolation { source, count } => {
                write!(
                    f,
                    "多重度制約違反: 辺 `{}` は {} {:?} について出次数 {} を期待しますが実際は {} 本です",
                    "担当", "人物", source, "0..1", count
                )
            }
...
pub struct Graph {
    ...
    >,
    担当: graphite::KeyedTable<担当Id, __担当Record>,
    /// 位置0キー -> このキーから (有向: 出る / 無向: 接続する) エッジ
    /// キーの一覧 (凍結時に構築)。
    担当_from_index: graphite::OptionalRoleIndex<__担当InternalPosition>,
    /// 位置1キー (終点) -> そこへ入るエッジキーの一覧 (凍結時に
    /// 構築。終点役割クエリの索引、`docs/reverse_query.md`)。
    担当_to_index: graphite::ExactlyOneRoleIndex<__担当InternalPosition>,
    __graphite_担当_by_pair: std::collections::HashMap<
        (__人物InternalPosition, __商品InternalPosition),
        Vec<__担当InternalPosition>,
    >,
...
impl Graph {
    ...
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当_by_id<'graph>(
        &'graph self,
        id: &担当Id,
    ) -> Option<担当Ref<'graph>> {
        Some(担当Ref {
            graph: self,
            internal_position: __担当InternalPosition(self.担当.position(id)?),
        })
    }
    /// この種別の辺の公開IDを挿入順に走査する。
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当_ids<'graph>(&'graph self) -> impl Iterator<Item = &'graph 担当Id> {
        self.担当.ids()
    }
    /// この種別の辺個体を挿入順に走査する。追加確保はしない。
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当_iter<'graph>(
        &'graph self,
    ) -> impl Iterator<Item = 担当Ref<'graph>> + 'graph {
        self.担当
            .positions()
            .map(move |position| 担当Ref {
                graph: self,
                internal_position: __担当InternalPosition(position),
            })
    }
    /// この種別の辺の件数を返す。
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当_len(&self) -> usize {
        self.担当.len()
    }
    /// 公開IDから完成済みグラフ上の辺個体を平均 O(1) で引く。
    ///
...
impl<'graph> 購入Ref<'graph> {
    fn record(self) -> &'graph __購入Record {
        let Some((_, record)) = self.graph.購入.get_at(self.internal_position.0) else {
            panic!(
    ...
    pub fn id(self) -> &'graph 購入Id {
        let Some((id, _)) = self.graph.購入.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
...
/// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
#[derive(Clone, Copy)]
pub struct 担当Ref<'graph> {
    graph: &'graph Graph,
    internal_position: __担当InternalPosition,
}
impl<'graph> 担当Ref<'graph> {
    fn record(self) -> &'graph __担当Record {
        let Some((_, record)) = self.graph.担当.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
    ...
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn id(self) -> &'graph 担当Id {
        let Some((id, _)) = self.graph.担当.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
    ...
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当者(self) -> 人物Ref<'graph> {
        人物Ref {
            graph: self.graph,
            internal_position: __人物InternalPosition(self.record().担当者.0),
        }
    }
    /// この辺個体の終点側の端点を役割名で返す。
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当商品(self) -> 商品Ref<'graph> {
        商品Ref {
            graph: self.graph,
            internal_position: __商品InternalPosition(self.record().担当商品.0),
        }
    }
    /// この辺個体の始点側の端点を、役割名によらない固定名で返す。
    ///
...
impl<'graph> std::fmt::Debug for 担当Ref<'graph> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct(stringify!(担当Ref))
            .field("id", &self.id())
            .finish_non_exhaustive()
    }
}
...
impl<'graph> 友人Ref<'graph> {
    fn record(self) -> &'graph __友人Record {
        let Some((_, record)) = self.graph.友人.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
    ...
    pub fn id(self) -> &'graph 友人Id {
        let Some((id, _)) = self.graph.友人.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
...
pub trait 複製世界Insertable: Sized {
...
pub trait 複製世界DefaultId: 複製世界Insertable {
    #[doc(hidden)]
...
/// 直接呼ぶことは想定しない。
pub trait 複製世界Node: 複製世界Insertable {}
impl 複製世界Insertable for super::人物 {
    type Id = 人物Id;
...
impl 複製世界DefaultId for super::人物 {
    fn insert_named_with_binding(
        self,
        b: &mut Builder,
        binding: String,
        ...
        複製世界Insertable::insert_named_with_id(self, b, 人物Id(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製世界Insertable::insert_with_id(self, b, 人物Id(binding))
    }
}
impl 複製世界Node for super::人物 {}
/// 完成済みグラフ上の `人物` ノード個体。
///
/// 宣言: `tests/graph_clone.rs` の `node 人物`
#[derive(Clone, Copy)]
...
impl<'graph> 人物Ref<'graph> {
    ...
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当_as_担当者(self) -> Option<担当Ref<'graph>> {
        self.graph
            .担当_from_index
            .get(self.internal_position.0)
            .copied()
            .map(|internal_position| 担当Ref {
                graph: self.graph,
                internal_position,
            })
    }
    ...
    > {
        ...
        Ok(
            positions
                .iter()
                .copied()
                .map(move |internal_position| 担当Ref {
                    graph: self.graph,
                    internal_position,
                }),
        )
    ...
    /// # Panics
    /// 2つの参照が異なる `Graph` から得られた場合にパニックする。
    /// パニックを避けたい場合は対の [`Self::担当_try_between`] を使う。
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当_between(
        self,
        ...
        self.担当_try_between(other)
            .unwrap_or_else(|error| {
                panic!(
                    "{}::{}: {error}", stringify!(人物Ref), stringify!(担当_between)
                )
            })
    }
...
impl 複製世界Insertable for super::商品 {
    type Id = 商品Id;
...
impl 複製世界DefaultId for super::商品 {
    fn insert_named_with_binding(
        self,
        b: &mut Builder,
        binding: String,
        ...
        複製世界Insertable::insert_named_with_id(self, b, 商品Id(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製世界Insertable::insert_with_id(self, b, 商品Id(binding))
    }
}
impl 複製世界Node for super::商品 {}
/// 完成済みグラフ上の `商品` ノード個体。
///
/// 宣言: `tests/graph_clone.rs` の `node 商品`
#[derive(Clone, Copy)]
...
impl<'graph> 商品Ref<'graph> {
    ...
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当_as_担当商品(self) -> 担当Ref<'graph> {
        担当Ref {
            graph: self.graph,
            internal_position: *self.graph.担当_to_index.get(self.internal_position.0),
        }
    }
}
...
/// (`{Builder}::add` 経由で使う)。
pub trait 複製世界Edge: 複製世界Insertable {}
impl 複製世界Insertable for 購入 {
    type Id = 購入Id;
...
impl 複製世界DefaultId for 購入 {
    fn insert_named_with_binding(
        ...
        複製世界Insertable::insert_named_with_id(self, b, 購入Id(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製世界Insertable::insert_with_id(self, b, 購入Id(binding))
    }
}
impl 複製世界Edge for 購入 {}
impl 複製世界Insertable for 担当 {
    type Id = 担当Id;
    type NamedPosition = __担当NamedPosition;
    fn insert_named_with_id(
        self,
        b: &mut Builder,
        id: Self::Id,
        ...
        let named_position = __担当NamedPosition(
            __担当InternalPosition(
                graphite::TablePosition::from_index(b.担当.len()),
            ),
            b.__graphite_construction_stamp,
        );
        let returned_id = id.clone();
...
impl graphite::NamedGraphElement<Graph> for __担当NamedPosition {
    type Reference<'graph> = 担当Ref<'graph>;
    fn bind<'graph>(&self, graph: &'graph Graph) -> Self::Reference<'graph> {
        if graph.__graphite_construction_stamp != self.1 {
            panic!(
                "名前付き位置が生成元と異なる Graph へ bind されました。名前付き位置は生成元の graph! が返したグラフでのみ有効です"
...
impl 複製世界DefaultId for 担当 {
    fn insert_named_with_binding(
        ...
        複製世界Insertable::insert_named_with_id(self, b, 担当Id(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製世界Insertable::insert_with_id(self, b, 担当Id(binding))
    }
}
impl 複製世界Edge for 担当 {}
impl 複製世界Insertable for 友人 {
    type Id = 友人Id;
...
impl 複製世界DefaultId for 友人 {
    fn insert_named_with_binding(
        ...
        複製世界Insertable::insert_named_with_id(self, b, 友人Id(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        複製世界Insertable::insert_with_id(self, b, 友人Id(binding))
    }
}
impl 複製世界Edge for 友人 {}
impl Builder {
    fn new() -> Self {
        Self {
    ...
    ///
    /// 宣言: `tests/graph_clone.rs` の `edge 担当 = (担当者: 人物) -> (担当商品: 商品) where each 担当商品: 1, each 担当者: 0..1`
    pub fn 担当(&mut self, id: 担当Id, value: 担当) -> &mut Self {
        self.担当.push((id, value));
        self
    }
    /// この種別の辺を公開IDと辺値の組で追加する。検査は凍結時に行う。
    ///
    ...
    /// `graph!` を経由しない。
    pub fn insert_with_id<N: 複製世界Node>(&mut self, id: N::Id, value: N) -> N::Id {
    ...
    /// (許可証は通常の `create` 経路からの直接的・偶発的な誤用を防ぐためのものであり、名前付き位置の持ち出しの検出は構築印の照合が担う。`crates/graphite/src/schema_runtime/named_construction.rs` 参照)。
    #[doc(hidden)]
    pub fn insert_named_with_id<N: 複製世界Node>(
        &mut self,
        id: N::Id,
        value: N,
        permit: &graphite::NamedInsertPermit,
    ...
    /// `graph!` を経由しない。
    pub fn add_with_id<E: 複製世界Edge>(&mut self, id: E::Id, value: E) -> E::Id {
        value.insert_with_id(self, id)
    }
    ...
    /// (許可証は通常の `create` 経路からの直接的・偶発的な誤用を防ぐためのものであり、名前付き位置の持ち出しの検出は構築印の照合が担う。`crates/graphite/src/schema_runtime/named_construction.rs` 参照)。
    #[doc(hidden)]
    pub fn add_named_with_id<E: 複製世界Edge>(
        &mut self,
        id: E::Id,
        value: E,
        permit: &graphite::NamedInsertPermit,
    ...
    fn freeze_collecting(self) -> Result<Graph, Vec<Violation>> {
        ...
        let mut __graphite_担当: graphite::KeyedTable<_, _> = graphite::KeyedTable::new();
        let mut __seen_edge_ids = std::collections::HashSet::new();
        let mut 担当_from_index: std::collections::HashMap<_, Vec<_>> = std::collections::HashMap::new();
        let mut 担当_to_index: std::collections::HashMap<_, Vec<_>> = std::collections::HashMap::new();
        let mut __graphite_担当_by_pair: std::collections::HashMap<
            (__人物InternalPosition, __商品InternalPosition),
            Vec<__担当InternalPosition>,
        > = std::collections::HashMap::new();
        for (id, value) in self.担当 {
            if !__seen_edge_ids.insert(id.clone()) {
                __violations.push(Violation::担当DuplicateKey(id));
                continue;
            }
            let 担当 { 担当者: from, 担当商品: to } = value;
            let from_position = __graphite_node_人物
                .position(&from)
                .map(__人物InternalPosition);
            let to_position = __graphite_node_商品
            ...
            if from_position.is_none() {
                __violations
                    .push(Violation::担当UnknownSource {
                        edge: id.clone(),
                        source: from.clone(),
                    });
            }
            if to_position.is_none() {
                __violations
                    .push(Violation::担当UnknownTarget {
                        edge: id.clone(),
                        target: to.clone(),
                    });
            }
            if let (Some(from_position), Some(to_position)) = (
                ...
                let internal_edge_position = __担当InternalPosition(
                    graphite::TablePosition::from_index(__graphite_担当.len()),
                );
                __graphite_担当_by_pair
        ...
        for position in __graphite_node_商品.positions() {
            let internal_position = __商品InternalPosition(position);
            let Some((key, _)) = __graphite_node_商品.get_at(position) else {
                panic!("列挙した内部位置はノード表に存在する")
            };
            let count = 担当_to_index
                .get(&internal_position)
                .map(Vec::len)
                .unwrap_or(0);
            if count != 1usize {
                __violations
                    .push(Violation::担当担当商品EachViolation {
                        target: key.clone(),
                        count,
                    });
            }
        ...
        for position in __graphite_node_人物.positions() {
            ...
            let Some((key, _)) = __graphite_node_人物.get_at(position) else {
                panic!("列挙した内部位置はノード表に存在する")
            };
            let count = 担当_from_index
                .get(&internal_position)
                .map(Vec::len)
                .unwrap_or(0);
            if !(0usize..=1usize).contains(&count) {
                __violations
                    .push(Violation::担当担当者EachViolation {
                        source: key.clone(),
                        count,
                    });
            }
        ...
        let 担当_from_index = graphite::OptionalRoleIndex::from_buckets(
            __graphite_node_人物
                .positions()
                .map(|position| {
                    担当_from_index
        ...
        let 担当_to_index = graphite::ExactlyOneRoleIndex::from_buckets(
            __graphite_node_商品
                .positions()
                .map(|position| {
                    担当_to_index
