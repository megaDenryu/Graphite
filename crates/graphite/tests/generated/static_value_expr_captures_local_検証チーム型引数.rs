...
// 生成元: tests/static_value_expr_captures_local.rs:98
...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    10882087137765790842u64, 11185478483943172401u64, 4581172880183613792u64,
    15163756864813571220u64,
];
...
pub mod construct {
    ...
    macro_rules! nodes {
        () => {
            { let (太郎, 次郎,) = __graphite_values_検証チーム型引数!();
            検証チーム型引数::Nodes::__graphite_internal_new(太郎, 次郎) }
        };
    }
    pub(crate) use nodes;
    ...
    macro_rules! edges {
        ($nodes:expr) => {
            { let (太郎の上司,) = __graphite_payloads_検証チーム型引数!();
            検証チーム型引数::Edges::__graphite_internal_new($nodes,
            太郎の上司) }
        };
    }
    pub(crate) use edges;
