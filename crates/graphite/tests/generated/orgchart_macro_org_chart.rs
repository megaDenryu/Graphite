...
// 生成元: crates/graphite/tests/orgchart_macro.rs:44
...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    1054794311358039144u64, 13417147945048160693u64, 9613789303992319322u64,
    17110079624498114430u64,
];
...
pub enum Violation {
    ...
    /// このエッジが未知の始点キーを参照している。
    ReportsUnknownSource { edge: ReportsId, source: EmployeeId },
    /// このエッジが未知の終点キーを参照している。
    ReportsUnknownTarget { edge: ReportsId, target: EmployeeId },
    /// このエッジ種別の `unique pair` 違反 (同じ始点・終点の対に
    /// 2本目の辺が張られた)。
    ReportsUniquePairViolation { source: EmployeeId, target: EmployeeId },
    /// このエッジ種別のキーが重複している。
    LeadsDuplicateKey(LeadsId),
    /// このエッジが未知の始点キーを参照している。
    LeadsUnknownSource { edge: LeadsId, source: EmployeeId },
    /// このエッジが未知の終点キーを参照している。
    LeadsUnknownTarget { edge: LeadsId, target: DepartmentId },
    /// このエッジ種別の `each` 制約違反 (入次数)。
    LeadsDepartmentEachViolation { target: DepartmentId, count: usize },
}
...
impl OrgChartInsertable for Leads {
    ...
    fn insert_named_with_id(
        ...
        let named_position = __LeadsNamedPosition(
            __LeadsInternalPosition(graphite::TablePosition(b.leads.len())),
            b.__graphite_construction_stamp,
        );
        let returned_id = id.clone();
        b.leads(id, self);
        ...
        for (id, value) in self.leads {
            ...
            if let (Some(from_position), Some(to_position)) = (
                ...
                let internal_edge_position = __LeadsInternalPosition(
                    graphite::TablePosition(__graphite_leads.len()),
                );
                __graphite_leads_by_pair
                    .entry((from_position, to_position))
                    .or_default()
