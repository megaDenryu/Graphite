...
// 生成元: tests/orgchart_macro.rs:49
...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    11870866092744501079u64, 8707300432953632152u64, 2759775096046148405u64,
    8151115718460102801u64,
];
/// `Employee` ノードの公開ID。
///
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
struct __LeadsInternalPosition(graphite::TablePosition);
#[doc(hidden)]
#[derive(Clone, Copy)]
...
impl graphite::DirectedEdgeLiteral<EmployeeId, EmployeeId, ()> for Reports {
    fn from_graph_literal(from: EmployeeId, to: EmployeeId, (): ()) -> Self {
        Self::new(from, to)
    }
}
...
impl graphite::DirectedEdgeLiteral<EmployeeId, DepartmentId, ()> for Leads {
    fn from_graph_literal(from: EmployeeId, to: DepartmentId, (): ()) -> Self {
        Self::new(from, to)
    }
}
impl std::fmt::Debug for Leads {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple(stringify!(Leads))
            .field(&self.leader)
            .field(&self.department)
            .finish()
    }
...
struct __ReportsRecord {
    reporter: __EmployeeInternalPosition,
    recipient: __EmployeeInternalPosition,
}
#[allow(dead_code)]
struct __LeadsRecord {
...
impl std::fmt::Display for Violation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Violation::LeadsDuplicateKey(id) => {
                write!(f, "{}のキーが重複しています: {:?}", "Leads", id)
            }
            ...
            Violation::LeadsDepartmentEachViolation { target, count } => {
                write!(
                    f,
                    "多重度制約違反: 辺 `{}` は {} {:?} について入次数 {} を期待しますが実際は {} 本です",
                    "Leads", "Department", target, "0..1", count
                )
            }
        }
    }
...
pub struct Graph {
    ...
    >,
    leads: graphite::KeyedTable<LeadsId, __LeadsRecord>,
    /// 位置0キー -> このキーから (有向: 出る / 無向: 接続する) エッジ
    /// キーの一覧 (凍結時に構築)。
    leads_from_index: graphite::MultipleRoleIndex<__LeadsInternalPosition>,
    /// 位置1キー (終点) -> そこへ入るエッジキーの一覧 (凍結時に
    /// 構築。終点役割クエリの索引、`docs/reverse_query.md`)。
    leads_to_index: graphite::OptionalRoleIndex<__LeadsInternalPosition>,
    __graphite_leads_by_pair: std::collections::HashMap<
        (__EmployeeInternalPosition, __DepartmentInternalPosition),
        Vec<__LeadsInternalPosition>,
    >,
...
impl<'graph> ReportsRef<'graph> {
    fn record(self) -> &'graph __ReportsRecord {
        let Some((_, record)) = self.graph.reports.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
    ...
    pub fn id(self) -> &'graph ReportsId {
        let Some((id, _)) = self.graph.reports.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
...
pub struct LeadsRef<'graph> {
    graph: &'graph Graph,
    internal_position: __LeadsInternalPosition,
}
impl<'graph> LeadsRef<'graph> {
    fn record(self) -> &'graph __LeadsRecord {
        let Some((_, record)) = self.graph.leads.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
    ...
    pub fn id(self) -> &'graph LeadsId {
        let Some((id, _)) = self.graph.leads.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
...
impl<'graph> std::fmt::Debug for LeadsRef<'graph> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct(stringify!(LeadsRef))
            .field("id", &self.id())
            .finish_non_exhaustive()
    }
}
...
impl OrgChartInsertable for Reports {
    type Id = ReportsId;
...
impl OrgChartDefaultId for Reports {
    fn insert_named_with_binding(
        self,
        b: &mut Builder,
        binding: String,
        ...
        OrgChartInsertable::insert_named_with_id(self, b, ReportsId(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        OrgChartInsertable::insert_with_id(self, b, ReportsId(binding))
    }
}
impl OrgChartEdge for Reports {}
impl OrgChartInsertable for Leads {
    type Id = LeadsId;
    type NamedPosition = __LeadsNamedPosition;
    fn insert_named_with_id(
        self,
        b: &mut Builder,
        id: Self::Id,
        ...
        let named_position = __LeadsNamedPosition(
            __LeadsInternalPosition(graphite::TablePosition::from_index(b.leads.len())),
            b.__graphite_construction_stamp,
        );
        let returned_id = id.clone();
        b.leads(id, self);
...
impl graphite::NamedGraphElement<Graph> for __LeadsNamedPosition {
    type Reference<'graph> = LeadsRef<'graph>;
    fn bind<'graph>(&self, graph: &'graph Graph) -> Self::Reference<'graph> {
        if graph.__graphite_construction_stamp != self.1 {
            panic!(
                "名前付き位置が生成元と異なる Graph へ bind されました。名前付き位置は生成元の graph! が返したグラフでのみ有効です"
...
impl OrgChartDefaultId for Leads {
    fn insert_named_with_binding(
        self,
        b: &mut Builder,
        binding: String,
        ...
        OrgChartInsertable::insert_named_with_id(self, b, LeadsId(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        OrgChartInsertable::insert_with_id(self, b, LeadsId(binding))
    }
}
impl OrgChartEdge for Leads {}
impl Builder {
    fn new() -> Self {
        Self {
    ...
    fn freeze_collecting(self) -> Result<Graph, Vec<Violation>> {
        ...
        let mut __graphite_leads: graphite::KeyedTable<_, _> = graphite::KeyedTable::new();
        let mut __seen_edge_ids = std::collections::HashSet::new();
        let mut leads_from_index: std::collections::HashMap<_, Vec<_>> = std::collections::HashMap::new();
        let mut leads_to_index: std::collections::HashMap<_, Vec<_>> = std::collections::HashMap::new();
        let mut __graphite_leads_by_pair: std::collections::HashMap<
            (__EmployeeInternalPosition, __DepartmentInternalPosition),
            Vec<__LeadsInternalPosition>,
        > = std::collections::HashMap::new();
        for (id, value) in self.leads {
            if !__seen_edge_ids.insert(id.clone()) {
                __violations.push(Violation::LeadsDuplicateKey(id));
                continue;
            }
            let Leads { leader: from, department: to } = value;
            let from_position = __graphite_node_employee
                .position(&from)
                .map(__EmployeeInternalPosition);
            let to_position = __graphite_node_department
            ...
            if from_position.is_none() {
                __violations
                    .push(Violation::LeadsUnknownSource {
                        edge: id.clone(),
                        source: from.clone(),
                    });
            }
            if to_position.is_none() {
                __violations
                    .push(Violation::LeadsUnknownTarget {
                        edge: id.clone(),
                        target: to.clone(),
                    });
            }
            if let (Some(from_position), Some(to_position)) = (
                ...
                let internal_edge_position = __LeadsInternalPosition(
                    graphite::TablePosition::from_index(__graphite_leads.len()),
                );
                __graphite_leads_by_pair
                    .entry((from_position, to_position))
                    .or_default()
                ...
                leads_to_index
                    ...
                    .or_default()
                    .push(internal_edge_position);
                let inserted = __graphite_leads
                    .insert(
                        id,
                        __LeadsRecord {
                            leader: from_position,
                            department: to_position,
                        },
                    );
                debug_assert!(inserted, "重複辺IDは挿入前に除外済み");
        ...
        for position in __graphite_node_department.positions() {
            ...
            let count = leads_to_index
                .get(&internal_position)
                .map(Vec::len)
                .unwrap_or(0);
            if !(0usize..=1usize).contains(&count) {
                __violations
                    .push(Violation::LeadsDepartmentEachViolation {
                        target: key.clone(),
                        count,
                    });
            }
        ...
        let leads_from_index = graphite::MultipleRoleIndex::from_buckets(
            __graphite_node_employee
                .positions()
                .map(|position| {
                    leads_from_index
        ...
        let leads_to_index = graphite::OptionalRoleIndex::from_buckets(
            __graphite_node_department
                .positions()
                .map(|position| {
                    leads_to_index
