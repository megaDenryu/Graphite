...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    10168756894076881803u64, 13386523414387080138u64, 5198811715303515881u64,
    3732364838459415901u64,
];
/// `Person` ノードの公開ID。
///
...
impl<'graph> KnowsRef<'graph> {
    fn record(self) -> &'graph __KnowsRecord {
        let Some((_, record)) = self.graph.knows.get_at(self.internal_position.0) else {
            panic!(
    ...
    pub fn id(self) -> &'graph KnowsId {
        let Some((id, _)) = self.graph.knows.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
...
pub trait SpliceDemoDefaultId: SpliceDemoInsertable {
    #[doc(hidden)]
...
impl SpliceDemoDefaultId for super::Person {
    fn insert_named_with_binding(
        self,
        b: &mut Builder,
        binding: String,
        ...
        SpliceDemoInsertable::insert_named_with_id(self, b, PersonId(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        SpliceDemoInsertable::insert_with_id(self, b, PersonId(binding))
    }
}
...
/// (`{Builder}::add` 経由で使う)。
pub trait SpliceDemoEdge: SpliceDemoInsertable {}
impl SpliceDemoInsertable for Knows {
    type Id = KnowsId;
...
impl SpliceDemoDefaultId for Knows {
    fn insert_named_with_binding(
        self,
        b: &mut Builder,
        binding: String,
        ...
        SpliceDemoInsertable::insert_named_with_id(self, b, KnowsId(binding), permit)
    }
    fn insert_with_binding(self, b: &mut Builder, binding: String) -> Self::Id {
        SpliceDemoInsertable::insert_with_id(self, b, KnowsId(binding))
    }
}
impl SpliceDemoEdge for Knows {}
impl Builder {
    fn new() -> Self {
        Self {
    ...
    where
        N: SpliceDemoNode + SpliceDemoDefaultId,
    {
    ...
    /// `graph!` を経由しない。
    pub fn insert_with_id<N: SpliceDemoNode>(&mut self, id: N::Id, value: N) -> N::Id {
    ...
    /// (許可証は通常の `create` 経路からの直接的・偶発的な誤用を防ぐためのものであり、名前付き位置の持ち出しの検出は構築印の照合が担う。`crates/graphite/src/schema_runtime/named_construction.rs` 参照)。
    #[doc(hidden)]
    pub fn insert_named_with_id<N: SpliceDemoNode>(
        &mut self,
        id: N::Id,
        value: N,
        permit: &graphite::NamedInsertPermit,
    ...
    where
        E: SpliceDemoEdge + SpliceDemoDefaultId,
    {
        value.insert_with_binding(self, key.into())
    }
    ...
    /// `graph!` を経由しない。
    pub fn add_with_id<E: SpliceDemoEdge>(&mut self, id: E::Id, value: E) -> E::Id {
        value.insert_with_id(self, id)
    }
    ...
    /// (許可証は通常の `create` 経路からの直接的・偶発的な誤用を防ぐためのものであり、名前付き位置の持ち出しの検出は構築印の照合が担う。`crates/graphite/src/schema_runtime/named_construction.rs` 参照)。
    #[doc(hidden)]
    pub fn add_named_with_id<E: SpliceDemoEdge>(
        &mut self,
        id: E::Id,
        value: E,
        permit: &graphite::NamedInsertPermit,
