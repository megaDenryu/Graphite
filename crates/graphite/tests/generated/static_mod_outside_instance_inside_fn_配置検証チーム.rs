...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    2263142338805255498u64, 267744264068150567u64, 14031671538577739232u64,
    7215336477036758252u64,
];
...
impl Nodes {
    /// Graphite 静的グラフの個体実体の所有者 `Nodes` を構築する (Graphite の固定語彙)。全個体を宣言順の位置引数にそのまま取り、値の計算は行わない。値ありの個体をinstance宣言の式から計算して渡すのは `配置検証チームの個体を組み立てる` の役目。
    ///
    /// - graph: `配置検証チーム`
    /// - 引数 (宣言順): `太郎: 社員, 開発部: 部署`
    ///
...
pub struct Edges<'a> {
    ...
    pub 太郎の所属: 配置検証組織::所属Edge<'a>,
}
impl<'a> Edges<'a> {
    /// Graphite 静的グラフの辺実体の所有者 `Edges` を構築する (Graphite の固定語彙)。値の計算は行わない。積み荷ありの具体辺の値をinstance宣言の式から計算して渡すのは`配置検証チームの辺を組み立てる` の役目。
    ///
    /// - graph: `配置検証チーム`
    /// - 第1引数: `nodes: &Nodes`
    ///
