...
// 生成元: tests/static_same_individual_name_multiple_instances.rs:48
...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    12279668443728390916u64, 6786104207428048697u64, 1644604020882223798u64,
    276811487033409826u64,
];
...
pub struct Nodes {
    ...
    /// - 実体型: `部署`
    ///
    /// 宣言: `tests/static_same_individual_name_multiple_instances.rs` の `node 開発部: 部署 = ..`
    pub 開発部: 部署,
}
impl Nodes {
    /// Graphite 静的グラフの個体実体の所有者 `Nodes` を構築する (Graphite の固定語彙)。全個体を宣言順の位置引数にそのまま取り、値の計算は行わない。値ありの個体をinstance宣言の式から計算して渡すのは `検証チームaの個体を組み立てる` の役目。
    ///
    /// - graph: `検証チームa`
    /// - 引数 (宣言順): `太郎: 社員, 開発部: 部署`
    ///
...
pub struct Edges<'a> {
    ...
    /// - 辺種別: `所属`
    ///
    /// 宣言: `tests/static_same_individual_name_multiple_instances.rs` の `edge 太郎の所属 = 所属(太郎 -[..]-> 開発部)`
    pub 太郎の所属: 検証組織::所属Edge<'a>,
}
impl<'a> Edges<'a> {
    /// Graphite 静的グラフの辺実体の所有者 `Edges` を構築する (Graphite の固定語彙)。値の計算は行わない。積み荷ありの具体辺の値をinstance宣言の式から計算して渡すのは`検証チームaの辺を組み立てる` の役目。
    ///
    /// - graph: `検証チームa`
    /// - 第1引数: `nodes: &Nodes`
...
pub struct 太郎の所属Ref<'a> {
    pub(super) entity: &'a 検証組織::所属Edge<'a>,
    pub(super) nodes: &'a Nodes,
    pub(super) edges: &'a Edges<'a>,
}
impl<'a> 太郎の所属Ref<'a> {
    ...
    ///
    /// 関係する instance 宣言: `tests/static_same_individual_name_multiple_instances.rs` の `edge 太郎の所属 = 所属(太郎 -[..]-> 開発部)`
