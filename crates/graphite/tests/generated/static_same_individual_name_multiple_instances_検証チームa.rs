...
// 生成元: tests/static_same_individual_name_multiple_instances.rs:45
...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    13330552155799950332u64, 15292265270834712535u64, 14261193844886518358u64,
    18195575543709850274u64,
];
...
pub mod construct {
    ...
    macro_rules! nodes {
        () => {
            { let (太郎, 開発部,) = __graphite_values_検証チームa!();
            #[allow(deprecated)] let __graphite_nodes =
            検証チームa::Nodes::__graphite_internal_new(太郎, 開発部);
            __graphite_nodes }
        };
    }
    pub(crate) use nodes;
    ...
    macro_rules! edges {
        ($nodes:expr) => {
            { let (太郎の所属,) = __graphite_payloads_検証チームa!();
            #[allow(deprecated)] let __graphite_edges =
            検証チームa::Edges::__graphite_internal_new($nodes, 太郎の所属);
            __graphite_edges }
        };
    }
    pub(crate) use edges;
