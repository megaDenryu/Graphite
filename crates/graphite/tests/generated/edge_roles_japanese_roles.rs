...
// 生成元: crates/graphite/tests/edge_roles.rs:50
...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    13955888517704561861u64, 4078658004075334674u64, 2940014931104703231u64,
    14268238267521562867u64,
];
...
pub enum Violation {
    ...
    OwnershipUnknownSource { edge: OwnershipId, source: PersonId },
    /// このエッジが未知の終点キーを参照している。
    OwnershipUnknownTarget { edge: OwnershipId, target: ItemId },
    /// このエッジ種別の `each` 制約違反 (出次数)。
    Ownership所有者EachViolation { source: PersonId, count: usize },
}
...
impl Item {
    pub fn get<'graph>(g: &'graph Graph, id: &ItemId) -> Option<ItemRef<'graph>> {
        let internal_position = __ItemInternalPosition(
            g.__graphite_node_item.position(id)?,
        );
        Some(ItemRef {
            graph: g,
            internal_position,
    ...
    pub fn get_mut<'graph>(
        g: &'graph mut Graph,
        id: &ItemId,
    ) -> Option<&'graph mut super::Item> {
        g.__graphite_node_item.get_mut(id)
    }
    pub fn ids<'graph>(g: &'graph Graph) -> impl Iterator<Item = &'graph ItemId> {
        g.__graphite_node_item.ids()
    }
    pub fn iter<'graph>(
        g: &'graph Graph,
    ) -> impl Iterator<Item = ItemRef<'graph>> + 'graph {
        g.__graphite_node_item
            .positions()
...
impl Ownership {
    ...
    pub fn of_所有者<'g>(node: PersonRef<'g>) -> OwnershipRef<'g> {
        OwnershipRef {
            graph: node.graph,
    ...
    pub fn of_所有物<'g>(
        node: ItemRef<'g>,
    ) -> impl Iterator<Item = OwnershipRef<'g>> + 'g {
        let positions = node.graph.ownership_to_index.get(node.internal_position.0);
        positions
            .iter()
            .copied()
    ...
    pub fn try_between<'g>(
        a: PersonRef<'g>,
        b: ItemRef<'g>,
    ) -> Result<impl Iterator<Item = OwnershipRef<'g>> + 'g, graphite::GraphMismatch> {
    ...
    pub fn between<'g>(
        ...
        Self::try_between(a, b)
            .unwrap_or_else(|error| {
                panic!("{}::between: {error}", stringify!(Ownership))
            })
    }
    pub fn get<'g>(g: &'g Graph, id: &OwnershipId) -> Option<OwnershipRef<'g>> {
        Some(OwnershipRef {
            graph: g,
            internal_position: __OwnershipInternalPosition(g.ownership.position(id)?),
        })
    }
    pub fn iter<'g>(g: &'g Graph) -> impl Iterator<Item = OwnershipRef<'g>> + 'g {
        g.ownership
            .positions()
    ...
    pub fn ids(g: &Graph) -> impl Iterator<Item = &OwnershipId> {
        g.ownership.ids()
    }
