...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    6724587475835172850u64, 2029250995484492839u64, 2442185455189506380u64,
    999629182253025976u64,
];
/// `Knows` 辺の公開ID。
///
...
impl std::fmt::Display for Violation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Violation::KnowsUnknownSource { edge, .. } => {
                write!(
                    f,
                    "未知のキーが {} として見つかりません (辺 `{}` {:?} の{}。端点のキーは表示できない)",
                    "Person", "Knows", edge, "始点"
                )
            }
            Violation::KnowsUnknownTarget { edge, .. } => {
                write!(
                    f,
                    "未知のキーが {} として見つかりません (辺 `{}` {:?} の{}。端点のキーは表示できない)",
                    "Person", "Knows", edge, "終点"
                )
            }
        }
    }
