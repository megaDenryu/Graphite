...
// 生成元: tests/static_same_individual_name_multiple_instances.rs:63
...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    17171864835202183388u64, 12501013240665857073u64, 10071004670865850870u64,
    5816180049160080690u64,
];
...
impl Nodes {
    /// Graphite 静的グラフの個体実体の所有者 `Nodes` を構築する (Graphite の固定語彙)。全個体を宣言順の位置引数にそのまま取り、値の計算は行わない。値ありの個体をinstance宣言の式から計算して渡すのは `検証チームbの個体を組み立てる` の役目。
    ///
    /// - graph: `検証チームb`
    /// - 引数 (宣言順): `太郎: 社員, 総務部: 部署`
    ///
...
impl<'a> Edges<'a> {
    /// Graphite 静的グラフの辺実体の所有者 `Edges` を構築する (Graphite の固定語彙)。値の計算は行わない。積み荷ありの具体辺の値をinstance宣言の式から計算して渡すのは`検証チームbの辺を組み立てる` の役目。
    ///
    /// - graph: `検証チームb`
    /// - 第1引数: `nodes: &Nodes`
