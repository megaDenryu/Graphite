...
// 生成元: tests/static_value_expr_name_resolves_at_call_site.rs:73
...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    1515103572946368434u64, 11715017720912715775u64, 5573250483444105380u64,
    10694030762349845736u64,
];
...
pub mod construct {
    ...
    ///
    /// 関係する instance 宣言: `tests/static_value_expr_name_resolves_at_call_site.rs` の `graph 名前解決チーム関数内`
    macro_rules! nodes {
        () => {
            { let (太郎, 開発部,) =
            __graphite_values_名前解決チーム関数内!(); #[allow(deprecated)] let
            __graphite_nodes =
            名前解決チーム関数内::Nodes::__graphite_internal_new(太郎,
    ...
    macro_rules! edges {
        ($nodes:expr) => {
            { let () = __graphite_payloads_名前解決チーム関数内!();
            #[allow(deprecated)] let __graphite_edges =
            名前解決チーム関数内::Edges::__graphite_internal_new($nodes,);
            __graphite_edges }
        };
    }
    pub(crate) use edges;
