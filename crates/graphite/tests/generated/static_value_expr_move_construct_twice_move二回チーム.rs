...
pub(super) const __GRAPHITE_STATIC_INSTANCE_FINGERPRINT: [u64; 4] = [
    11148195142803831055u64, 18415716628436066154u64, 1916755001018723769u64,
    9213260113299899189u64,
];
...
pub mod construct {
    ...
    macro_rules! nodes {
        () => {
            { let (太郎, 開発部,) = __graphite_values_move二回チーム!();
            #[allow(deprecated)] let __graphite_nodes =
            move二回チーム::Nodes::__graphite_internal_new(太郎, 開発部);
            __graphite_nodes }
        };
    }
    pub(crate) use nodes;
    ...
    macro_rules! edges {
        ($nodes:expr) => {
            { let () = __graphite_payloads_move二回チーム!(); #[allow(deprecated)]
            let __graphite_edges =
            move二回チーム::Edges::__graphite_internal_new($nodes,);
            __graphite_edges }
        };
    }
    pub(crate) use edges;
