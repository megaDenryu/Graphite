...
// 生成元: tests/node_id_shared_across_schemas.rs:105
...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    16963467886085734398u64, 17831904425476309937u64, 18137008181705401980u64,
    13590310146919482064u64,
];
/// `Approves` 辺の公開ID。
///
...
impl<'graph> ApprovesRef<'graph> {
    fn record(self) -> &'graph __ApprovesRecord {
        let Some((_, record)) = self.graph.approves.get_at(self.internal_position.0)
        else {
            panic!(
    ...
    pub fn id(self) -> &'graph ApprovesId {
        let Some((id, _)) = self.graph.approves.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
