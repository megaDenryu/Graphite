...
//! のファイルは検証対象1つ (ノード参照と辺参照) に対するテスト用スキーマとテ
//! スト関数の列を持つ。このファイルはテスト用の `graph_schema!` 宣言を同居さ
//! せているため、分けると同じ宣言が各ファイルへ複製される。超過を許す根拠の台
//! 帳は `docs/development/line_count_ledger.md` にある。
