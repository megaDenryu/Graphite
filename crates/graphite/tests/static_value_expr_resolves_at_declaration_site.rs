...
//! ことを固定する回帰試験 (PR #45、`docs/static_graph.md`「値の式の名前
//! 解決」節)。以前の実装は、`construct::nodes!`/`construct::edges!`を
//! 呼んだ位置を起点に値の式の名前を解決しており、呼び出し位置に同じ名前の
//! 別の項目があれば意味がすり替わった (`static_value_expr_name_resolves_at_call_site.rs`
//! が固定していた旧仕様。PR #45でオーナーの裁定によりこの方向を廃した)。
//! この試験は、instance宣言の位置と構築マクロの呼び出し位置に同名の
//! 識別子があっても、instance宣言の位置の意味を保つことを確かめる。
