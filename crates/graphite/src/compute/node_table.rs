//! 計算ノード表 — キーからノード種別を読み出し、依存キー列と計算の実行を答える。
...
impl<V> 計算ノード表<V> {
    ...
    /// 計算ノードの依存キー列 (宣言順)。入力ノードには空を返す。
    ///
    /// # Panics
    /// `キー` がこの表に存在しない場合 (凍結時に検証済みのキーしか渡らない
    /// 呼び出し規約の違反)。
    pub(in crate::compute) fn 依存キー列(&self, キー: &str) -> &[String] {
        match self.種別.get(キー) {
            None => unreachable!("凍結時に検証済みのキーしか来ない"),
            Some(ノード種別::入力ノード) => &[],
            Some(ノード種別::計算ノード {
                依存キー列, ..
            }) => 依存キー列,
    ...
    /// 計算ノードの値を求める。入力ノードには `None` を返す (入力ノードの値は
    /// 書き込まれるものであって計算されるものではない)。
    ///
    /// # Panics
    /// `キー` がこの表に存在しない場合 (凍結時に検証済みのキーしか渡らない
    /// 呼び出し規約の違反)。
