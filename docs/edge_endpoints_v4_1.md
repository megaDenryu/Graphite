...
旧省略構文は互換として残さない。生成される辺値型・辺参照・索引の実際の形は
`docs/desugaring_reference.md` §4・§7・§11 にある。
