...
## 1.16 静的グラフの受理マトリクス (issue #41 段階5、2026-09-23。instance展開のimpl撤去に伴い2026-09-24に再測定。PR #45レビューA・C・D・Fの構築境界の刷新に伴いさらに再測定。PR #45検収の指摘1〜3・7の是正 (コミット`bb551a4`) に伴いさらに再測定)
...
| `g.node_refs.太郎` | `g.node_refs.太郎` の `太郎` (main.rs:111) | `generated/開発チーム.rs`の`NodeRefs`の`太郎`フィールド (774〜780行目、0始まり) | 「個体参照フィールド。NodeRefsがこの個体の具体参照を持つ」+ 宣言: `node 太郎: 社員 = ..` | 合格 |
| `g.edge_refs.太郎の所属` | `g.edge_refs.太郎の所属` の `太郎の所属` (main.rs:116) | `generated/開発チーム.rs`の`EdgeRefs`の`太郎の所属`フィールド (835〜841行目、0始まり) | 「辺参照フィールド」+ 宣言: `edge 太郎の所属 = 所属(太郎 -> 開発部)` | 合格 |
...
| 識別子 | F12起点 | F12の着地先 | 意味カードの要約 | 合否 |
|---|---|---|---|---|
| `construct::nodes!` | `開発チーム::construct::nodes!(` の呼び出し (main.rs:95、再実測: 2026-09-24 bb551a4) | `generated/開発チーム.rs`の`macro_rules! nodes`定義 (953〜969行目、0始まり。doc付きの項目全体) | 「個体実体の所有者`Nodes`を構築するマクロ`nodes`。graph: 開発チーム / 実行時に渡す個体 (宣言順): `開発部: 部署` / 戻り値: Nodes」+ 固定語彙: `construct::nodes!` + 関係する instance 宣言: `graph 開発チーム` | 合格 (生成ファイルの中に着地する。旧・組み立て関数の例外は解消した) |
| `construct::edges!` | `開発チーム::construct::edges!(` の呼び出し (main.rs:108、再実測: 2026-09-24 bb551a4。fn main()内の呼び出し) | `generated/開発チーム.rs`の`macro_rules! edges`定義 (971〜987行目、0始まり) | 「辺実体の所有者`Edges`を構築するマクロ`edges`。graph: 開発チーム / 引数: `nodes: &Nodes` / 戻り値: Edges」+ 固定語彙: `construct::edges!` + 関係する instance 宣言: `graph 開発チーム` | 合格 |
...
| `NodeRefs` | `pub node_refs: NodeRefs<'a>`の型参照 (`generated/開発チーム.rs:926`) | `generated/開発チーム.rs`の`struct NodeRefs<'a>`定義 (768〜802行目、0始まり) | 「個体参照の集まり `NodeRefs` (Graphiteの固定語彙)」+ 固定語彙: `NodeRefs` | 合格 |
| `EdgeRefs` | `pub edge_refs: EdgeRefs<'a>`の型参照 (`generated/開発チーム.rs:932`) | `generated/開発チーム.rs`の`struct EdgeRefs<'a>`定義 (829〜877行目、0始まり) | 「辺参照の集まり `EdgeRefs` (Graphiteの固定語彙)」+ 固定語彙: `EdgeRefs` | 合格 |
| `new` (`Graph::new`) | `開発チーム::Graph::new` の`new` (main.rs:109) | `generated/開発チーム.rs`の`impl<'a> Graph<'a>`の`new`メソッド (934〜945行目、0始まり) | 「`Graph`を構築する (Graphiteの固定語彙)」+ 固定語彙: `Graph::new` | 合格 |
...
| `node_refs` | `g.node_refs` の`node_refs` (main.rs:111) | `struct Graph<'a>`の`node_refs`フィールド (920〜925行目、0始まり) | 「`Graph`が持つ個体参照の集まりへのフィールド `node_refs` (Graphiteの固定語彙)」+ 固定語彙: `node_refs` | 合格 |
| `edge_refs` | `g.edge_refs` の`edge_refs` (main.rs:116) | `struct Graph<'a>`の`edge_refs`フィールド (926〜931行目、0始まり) | 「`Graph`が持つ辺参照の集まりへのフィールド `edge_refs` (Graphiteの固定語彙)」+ 固定語彙: `edge_refs` | 合格 |
