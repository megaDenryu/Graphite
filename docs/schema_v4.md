...
- **ノード型名は単純な識別子だけを書ける。** `node Employee;` の `Employee` に
  `crate::Employee` のようなモジュール修飾は書けない。ノード型名は `edge` 宣言の
  端点の型名とトークンの文字列として照合し、同じトークンが `node` 宣言と `edge`
...
違反は `Display` (`{}`) で表示される。`Debug` (`{:?}`) は `Display` へ委譲するため、
利用者はどちらで表示しても同じ文を受け取る。生成コードは文の先頭へ、見つからなかった
綴りを置く。未知の端点キーを参照した違反であれば次の形になり、辺のキーは末尾の括弧へ
回る。

```
未知のキー PersonId("ミオ") が Person として見つかりません (辺 `Knows` KnowsId("経路0") の始点)
```

利用者が、辺に識別子を持たない入力形式から構築する場合、辺のキーは利用者が機械的に
採番した値になり、入力ファイルの中で参照できる語ではない。そのため生成コードは、診断の
文の先頭に、入力ファイルの中の語である端点の綴りを置く (issue #26)。

違反が載せる綴りは、そのID型が生成ID型かどうかで決まる。生成コードが利用者の
`(id: 型パス)` で宣言した型へ `Debug` を要求しないため、宣言型のキーは文へ出ない。
生成コードは辺のキーと端点のキーの表示可否を別々に判定するので、片方だけが宣言型でも、
もう片方の綴りは文に残る。生成コードは、綴りを省いた側に「表示できない」と添える。
読み手が「表示できないのか、そもそも無いのか」を区別するためである。表示できない理由は、
利用者が `(id: 型パス)` で宣言した型へ生成コードが `Debug` を要求しないためである。

この診断には、利用者が知っておくべき限界がある。端点も辺も利用者が宣言したID型であれば、
診断には綴りが一切出ない。この場合、企画者は入力ファイルのどこを直すかを診断だけから
辿れない。辿れるようにするには、利用者が少なくとも端点のノードのID型を生成型にするか、
利用者が自分のID型へ `Debug` を実装する必要がある。
...
### 3.1.2 利用者が書く型へ求めるトレイト

生成コードは、利用者が書いた型へ必要最小限のトレイトしか要求しない。schema を書く前に
利用者が備えるべき実装は次の3種類だけである。

- **ノード値型** (`Person` 等) は何も実装しなくてよい。builder が値を受け取り、凍結が値を
  移動し、`NodeRef` が参照を返すだけであり、生成コードは値を複製も比較も表示もしない。
  これは schema が生成する API に関しての話である。利用者がノード値を水準1の
  `Graph<N, E, K>` へ載せて `filter_nodes` 等を呼ぶときは、その API が `N: Clone` を
  要求する (`crates/graphite/src/graph/mod.rs:321-336`)
- **積み荷型** (`BossEdge` 等) は `Clone` だけを実装する。生成コードは辺値を複製しないが、
  辺値は利用者が組み立てて使い回す入力値であり、`Clone` を備えることが Rust の直感に
  沿うため、生成コードは辺値型へ `Clone` を導出する。導出はフィールドの型へ同じトレイトを
  要求するため、積み荷型も `Clone` を実装する
- **ID型** は `Clone + Eq + Hash` を実装する。生成コードがノードと辺の表のキーとして使う
  ためである。`(id: 型パス)` で自分の型を指定したときは利用者が実装を用意する。利用者が
  ID型を省略して生成させたときは、生成コードが `Debug, Clone, PartialEq, Eq, Hash` を
  導出済みの newtype を生成する

**辺値型は `PartialEq` を要求しない。** 生成コードは辺値の等値比較を使わないため、
辺値型が `PartialEq` を導出するのは積み荷の無い辺だけである。生成コードが積み荷のある辺で
導出すると、積み荷の型が `PartialEq` の実装を強いられるためである。利用者は積み荷のある
辺値を比較するとき、端点IDと積み荷をそれぞれ取り出して比べる。

**辺値型の `Debug` は生成コードが手で実装する。** 積み荷が無く、かつ両端のID型が生成
された newtype である辺だけが端点を表示し、それ以外は種別名だけを表示する。この手書き
実装が利用者定義のID型と積み荷型へ `Debug` を要求しないためである。

**schema へ積み荷を足す編集は、追加に見えて辺値型の公開トレイトを減らす。** 開発者が
既存の辺へ積み荷を後から足すと、その辺値型は `PartialEq` と端点の `Debug` 表示を失う。
利用側で辺値を `==` で比べている箇所はコンパイルエラーになるため、利用者はその箇所を
端点IDと積み荷を個別に比べる形へ直す。

この要求は `verification/external-crate` が機械で確かめている。同パッケージのノード値型は
どのトレイトも導出せず、積み荷型は `Clone` だけを導出しており、開発者が導出を足すと保証が
消える。
