...
もう片方の綴りは文に残る。未知の端点キーと `unique pair` の違反では、生成コードは、
綴りを省いた側に「表示できない」と添える。読み手が「表示できないのか、そもそも無いのか」を
区別するためである。

この診断には、利用者が知っておくべき限界がある。端点も辺も利用者が宣言したID型であれば、
診断にはキーの綴りが一切出ない。この場合、利用者は入力ファイルのどこを直すかを診断だけから
辿れない。辿れるようにするには、利用者が端点のノードのID型を生成型にする必要がある。
