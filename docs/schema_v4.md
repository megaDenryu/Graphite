...
- **ノード値型** (`Person` 等) は何も実装しなくてよい。builder が値を受け取り、凍結が
  move し、`NodeRef` が参照を返すだけであり、生成コードは値を複製も比較も表示もしない
- **積み荷型** (`BossEdge` 等) は `Clone` だけを実装する。辺値型が `Clone` を導出して
  おり、導出はフィールドの型へ同じトレイトを要求するためである
- **ID型** は `Clone + Eq + Hash` を実装する。ノードと辺の表のキーとして使うためである。
  `(id: 型パス)` で自分の型を指定したときは利用者が実装を用意する。ID型を省略して生成
  させたときは `Debug, Clone, PartialEq, Eq, Hash` を導出済みの newtype が生成される

**辺値型は `PartialEq` を要求しない。** 生成コードは辺値の等値比較を使わないため、
辺値型が `PartialEq` を導出するのは積み荷の無い辺だけである。積み荷のある辺で導出すると
積み荷の型が `PartialEq` の実装を強いられるためである。積み荷のある辺値を比較するときは、
端点IDと積み荷をそれぞれ取り出して比べる。

**辺値型の `Debug` は生成コードが手で実装する。** 積み荷が無く、かつ両端のID型が生成
された newtype である辺だけが端点を表示し、それ以外は種別名だけを表示する。利用者定義の
ID型と積み荷型へ `Debug` を要求しないためである。

この要求は `verification/external-crate` が機械で確かめている。同パッケージのノード値型は
どのトレイトも導出せず、積み荷型は `Clone` だけを導出しており、導出を足すと保証が消える。
