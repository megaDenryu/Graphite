...
**積み荷型 (`BossEdge` 等) には Graphite 固有のトレイト要求が無い。** 生成器・ランタイムは
辺値を複製も比較もしない (凍結は辺値を `move` して処理する)。したがって積み荷を持つ辺の
構築用辺値型は `Clone` も `PartialEq` も導出しない。生成コードがこれらを導出すると、
Graphite が使わない複製可能性・比較可能性を積み荷の型へ要求することになるためである
(issue #27, #35)。利用者が同じ内容の辺を複数回作りたいときは、積み荷自身が `Clone` を
実装するなら利用側で積み荷を複製して新しい辺値を作るか、`move` して1回だけ使う。積み荷の
無い辺の構築用辺値型は端点の
公開ID型だけで構成され、その公開ID型が既に `Clone + Eq + Hash` を要求されているため、
`Clone` と `PartialEq` を導出しても利用者へ新しい要求を課さない。
...
既存の辺へ積み荷を後から足すと、その辺値型は `Clone`・`PartialEq`・端点の `Debug` 表示を
失う。利用側で辺値を `.clone()` していた箇所、`==` で比べていた箇所はコンパイルエラーに
なるため、利用者はその箇所を積み荷の複製または `move`、端点IDと積み荷を個別に比べる形へ直す。
