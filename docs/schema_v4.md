...
違反は `Display` (`{}`) で表示する。`Debug` (`{:?}`) は `Display` へ委譲するため、
利用者はどちらで表示しても同じ文を受け取る。文の先頭には、解決できなかった綴りが
出る。未知の端点キーを参照した違反であれば「未知のキー `PersonId("ミオ")` が Person
として解決できません (辺 `Knows` `KnowsId("経路0")` の始点)」の形になり、辺のキーは
末尾の括弧へ回る。辺に識別子を持たない入力形式から構築する場合、辺のキーは利用者が
機械的に採番した値になり原因の定義を指さないため、入力ファイルの中の語である端点の
綴りを先に読ませる (issue #26)。

違反が載せる綴りは、そのID型が生成ID型かどうかで決まる。利用者が `(id: 型パス)` で
宣言した型へ `Debug` を要求しないため、宣言型のキーは文へ出ない。辺のIDと端点のキーは
別々に判定するので、片方だけが宣言型でも、もう片方の綴りは文に残る。
