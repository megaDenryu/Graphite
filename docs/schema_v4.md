...
採番した値になり、入力ファイルの中で参照できる語ではない。そのため診断の文は、入力
ファイルの中の語である端点の綴りを先に置く (issue #26)。

違反が載せる綴りは、そのID型が生成ID型かどうかで決まる。生成コードが利用者の
`(id: 型パス)` で宣言した型へ `Debug` を要求しないため、宣言型のキーは文へ出ない。
生成コードは辺のIDと端点のキーの表示可否を別々に判定するので、片方だけが宣言型でも、
もう片方の綴りは文に残る。綴りを省いた側には、生成コードが省いた理由を添える。読み手が
「表示できないのか、そもそも無いのか」を区別できないためである。
