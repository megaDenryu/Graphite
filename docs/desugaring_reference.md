...
`dynamic_graph_schema!`/`graph!`/`flow!` の独自構文を消去したときにどの普通のRustの
ファイル・型・値・関数へ対応するかを、曖昧さなく示す正本である。チュートリアルでも
設計史でもない。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1-10`)。
...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    16180433103664140677u64, 18374092448019823346u64, 13270948032441293255u64,
    12970048397743723419u64,
];
```
...
渡すだけである (静的グラフのschema・instanceも同じ関数を使う。issue #41)。
...
一切行わない (`crates/graphite/tests/generated/edge_roles_commerce.rs:1276-1279`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:296`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:326-371`、署名のみ抜粋)。
...
分だけである (`crates/graphite/tests/generated/schema_ids_mixed_ids.rs:11-20`)。
...
(`crates/graphite/tests/generated/schema_ids_mixed_ids.rs:454-459`、署名のみ抜粋)。
...
(`crates/graphite/tests/generated/schema_ids_mixed_ids.rs:1161-1163`)。
...
表示も同じ規則に従う (`crates/graphite/tests/generated/schema_ids_mixed_ids.rs:285-292`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:52-94`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:95-125`)。
...
内部位置型と、凍結後の辺記録を生成する
(`crates/graphite/tests/generated/edge_roles_commerce.rs:36-37, 127-132`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1272-1278`)。端点の存在検査も多重度検査も
凍結まで行わない。

**6. 完成済みGraphの内部保存**

`Graph` は辺種別ごとに、辺表と役割索引と端点対索引を持つ
(`crates/graphite/tests/generated/edge_roles_commerce.rs:298-308`)。
...
辺種別APIは `Graph` に生える
(`crates/graphite/tests/generated/edge_roles_commerce.rs:418-465`、署名のみ抜粋)。
...
(`crates/graphite/tests/generated/role_query_rev_query.rs:1667-1725`)。
...
多重度違反のvariantも生成する (`crates/graphite/tests/generated/edge_roles_commerce.rs:163-176`)。
...
(`crates/graphite/tests/generated/edge_roles_japanese_roles.rs:107`)。
...
(`crates/graphite/tests/generated/role_query_rev_query.rs:443, 472, 483`)。
...
違反variantを1つ追加する (`crates/graphite/tests/generated/edge_roles_commerce.rs:177-184`)。
...
である (`crates/graphite/tests/generated/edge_roles_commerce.rs:305-319`)。
...
(`crates/graphite/tests/generated/undirected_edges_social.rs:41-68`)。
...
(`crates/graphite/tests/generated/undirected_edges_social.rs:78-108`)。
...
(`crates/graphite/tests/generated/undirected_edges_social.rs:138-153`)。
...
辺記録も順序なし対で保持する (`crates/graphite/tests/generated/undirected_edges_social.rs:119-127`)。
...
`UnorderedPair` になる (`crates/graphite/tests/generated/undirected_edges_social.rs:213-220`)。
...
両端を返す (`crates/graphite/tests/generated/undirected_edges_social.rs:422-437`)。
...
(`crates/graphite/tests/generated/undirected_edges_social.rs:654-667`)。
...
(`crates/graphite/tests/generated/undirected_edges_social.rs:668-689`)。
...
(`crates/graphite/tests/generated/schema_ids_mixed_ids.rs:396`)。

**7. 公開API**

既定IDと同じ。`Debug` の表示規則は §3 と同じで、明示ID型を含む辺値は種別名だけを
表示する (`crates/graphite/tests/generated/schema_ids_mixed_ids.rs:79-83`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:823-858`)。
...
`Deref` と `Debug` も生成する (`crates/graphite/tests/generated/edge_roles_commerce.rs:983-1003`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:543-626`)。
...
impl<'graph> PurchaseRef<'graph> {
    fn record(self) -> &'graph __PurchaseRecord {
        let Some((_, record)) = self.graph.purchase.get_at(self.internal_position.0)
        else {
            panic!(
    ...
    pub fn id(self) -> &'graph PurchaseId {
        let Some((id, _)) = self.graph.purchase.get_at(self.internal_position.0) else {
            panic!(
                "EdgeRefの内部位置は凍結後に不変の辺表を指す(生成元と異なるGraphへの束縛はbindの構築印照合で防いでいるため、ここに到達する場合は内部位置の不変条件が別の原因で破れている)"
            )
        };
...
脱糖する (`crates/graphite/tests/generated/edge_roles_commerce.rs:1325-1335`)。
...
作る (`crates/graphite/tests/generated/edge_roles_commerce.rs:812-824`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:39-50`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:795-808`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:776-791`)。
...
保持する (`crates/graphite/tests/generated/edge_roles_commerce.rs:320-323`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:524-530`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:507-541`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1678-1682`)。

`Builder` の公開メソッドは次のとおりである
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1264-1409`)。
...
置く (`crates/graphite/tests/generated/edge_roles_commerce.rs:740-754`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:715-726`)。`Builder::new` も非公開であり、
...
| 日本語の種別 `関係` と役割 `始点` | `PersonRef::関係_as_始点` (`crates/graphite/tests/generated/traversal_api_traversal.rs:1321`) |

`{kind}_between` / `{kind}_try_between` の主語は位置0側 (有向辺は始点側、無向辺は
唯一の端点型) の `NodeRef` である。

これらは手続き型マクロの展開の中に隠れておらず、生成ファイルに実在する
(`crates/graphite/tests/generated/edge_roles_commerce.rs:859-875`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1090-1104`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:876-917`)。
...
`crates/graphite/tests/generated/edge_roles_commerce.rs:341-346, 430-438` から署名のみ抜粋する。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:140-184`。`Subscription` 側の同型のvariantは省く)。
...
委譲する (`crates/graphite/tests/generated/edge_roles_commerce.rs:285-290`)。

**4. private生成物**

`Builder::freeze_collecting` と `Builder::freeze` はどちらも非公開である
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1414, 1672`)。凍結を外から呼ぶ入口は
`Graph::create` 系だけである。`graphite::build_named_graph` から具体型を知らずに
凍結を呼ぶための橋渡しだけが公開されている
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1700-1706`)。
...
`crates/graphite/tests/generated/edge_roles_commerce.rs:1414-1677` にある。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1485-1487`)。
...
確定形への変換は次の形である (`crates/graphite/tests/generated/edge_roles_commerce.rs:1637-1656`)。
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:1510-1512`)。`where each <役割名>` の役割名
...
(`crates/graphite/tests/generated/edge_roles_commerce.rs:291-295`)。
...
書く。判定はこの種類ごとに行っており、項目1件ずつでは行っていない。
...
上のうち `Builder` の挿入API・挿入トレイト・公開trait実装を除く公開型と公開
メソッドは、doc の末尾に宣言元を指す1段落を持つ (§26.6)。
...
いるので、そこから宣言へ戻れる。さらに、公開型と公開メソッドの doc が種別ごとの
宣言を指す (§26.6)。実測記録は `docs/development/ide_support_spec.md` §1.15 に
ある。
...
crates/graphite/tests/generated/edge_roles_commerce.rs の実装行
    ↓ その要素の doc の「宣言:」行 (パッケージ相対のファイルと宣言の形)
    ↓ またはファイル先頭の「生成元」コメント (パッケージ相対のファイルと行)
crates/graphite/ の tests/edge_roles.rs:27 にある schema 宣言
```

`graph!` の名前付きラッパーだけは例外で、呼び出し箇所ローカル型のため生成ファイルへ
事前生成しない。左辺名の集合が呼び出し箇所ごとに異なり、安定したmoduleファイルへ
書き出せないためである。追跡性はスパンの規約で担保する。`graph.alice()` の定義情報は
左辺の `alice` へ結び付ける。schemaに由来する公開APIをこの例外へ含めることはしない。

### 26.6 生成物の doc が指す宣言元

生成する公開型と公開メソッドは、doc の末尾に宣言元を指す1段落を持つ。書式は
「宣言: `<宣言元ファイルのパッケージ相対の綴り>` の `<宣言の形>`」である。

| 生成物 | 指す宣言 |
|---|---|
| `{Node}Id` / `{Node}Ref` / ノードの種別API (`{node}_by_id` 等) | その `node` 宣言 |
| `{Node}Ref` のアクセサ (`id` / `value`) | その `node` 宣言 |
| `Builder` の種別追加メソッド (`{node}`) | その `node` 宣言 |
| `{Kind}Id` / 辺値型 `{Kind}` / `{Kind}Ref` / 辺の種別API / `NodeRef` の探索メソッド | その `edge` 宣言 |
| 辺値型のメソッド (`new` / `payload` / `endpoints`) | その `edge` 宣言 |
| `{Kind}Ref` のアクセサ (`id` / 役割名 / `from` / `to` / `from_id` / `to_id` / `payload` / `endpoints`) | その `edge` 宣言 |
| `Builder` の種別追加メソッド (`{kind}`) | その `edge` 宣言 |
| `Graph` / `Builder` / `Violation` | `schema` 宣言 |

この段落を持たない公開生成物は3種類ある。schema全体の構築入口 (`Graph::create` /
`create_named` / `create_collecting`)、種別によらない `Builder` の挿入API
(`insert` / `add` / `extend` 系)、挿入トレイト (`{Schema}Insertable` /
`{Schema}DefaultId` / `{Schema}Node` / `{Schema}Edge`) である。いずれも種別ごとに
生成せず schema 1つに1組だけ生成するもので、これらが属する `Graph` と `Builder`
の側が既に `schema` 宣言を指しているため、同じ参照を重ねない。`#[doc(hidden)]` の
...
宣言の形は正規化した形であり、DSL 原文の逐語ではない。末尾の `;` は書かず、
明示ID型のパスは生成 module 内から解決できる形 (`self::X` は `super::X`) になり、
`where` 節は `each` を記述順に並べた後ろへ `unique pair` を回す。
...
なおこの文書の本文が引用するコードブロックは、抜粋を短く保つためこの「宣言:」の
段落を省いている。引用に添えた行番号は省略前の実物を指す。
