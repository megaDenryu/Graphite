...
- テスト (実測、端点宣言v4.1 [役割名+無向辺] 完了時点): コア87 (`crates/graphite`
  単体31 + 統合52 [`compile_fail`1・`explicit_plural_field`1・`f64_attrs`1・
  `graph_cross_module`1・`keyed_table_insertion_order`2・
  `orgchart_handwritten`8・`orgchart_macro`28・`orphan_node_no_warning`1・
  `undirected_edges`9 (新規)] + `graphite-macros`単体2 + doctest2) +
  examples 7本 (hello-graph 15 / build-pipeline 32 / org-analyzer 11 /
  dialogue-engine 14 / state-machine 15 / async-dag 15 /
  reactive-cells 25)、合計 214、全通過。`orgchart_macro` は役割名つき辺の
  アクセサ・入次数each制約のテスト追加で25→28に、reactive-cellsはSubの
  Lhs/Rhs書き直し (§3.11) で23→25に増加。制約なし辺 (平行辺) を持つ
  dialogue-engine・org-analyzer・hello-graph は複数回連続実行して非決定性が
  無いことも確認済み。構文は v4/v4.1 のみ (v1/v2/v3 は検出・移行診断なしで
  完全廃止)
- コミット (v3→v4移行分、古→新): `342ee0b` (v4設計文書) → `3fff112`
  ...
  (docs: hello-graph無向辺の最小例追加) → (本節・仕様書・モデリングガイド
  更新の docs コミットが続く)
- リモート: `origin/main` へ本セッション分含め push 済み (§3.11 時点で
  ローカル6コミット先行していたものを本docsコミットとまとめて反映)
