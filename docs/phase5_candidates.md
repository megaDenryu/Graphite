...
- **(h) `graph!` 内のノードキーが全ノード型で単一の平坦な名前空間** —
  ...
  → **診断+文書化で対応、名前空間再設計は見送り (フェーズ5)**。単一
  名前空間という制約自体はそのまま (型ごとに分ける再設計はコスト対効果が
  見合わないと判断)、代わりに同じ識別子を2回ノード宣言した場合に span
  付きの `syn::Error` (「識別子 `X` は既に宣言されています」+ 最初の
  宣言位置) を出すようにした (`instance_codegen::build_key_types`)。
  trybuild ケース (`tests/ui/graph_duplicate_node_key.rs`) を追加し、
  README「名前空間に関する制約」節に明記した。
- **(i) `(0..*)` エッジの列挙順がソース記述順で安定していることに依存した
  ...
  価値がある。
  (出典: `examples/dialogue-engine`)
  → **解決 (フェーズ5)**: 「同一始点キーに対する複数終点の相対順序は
  構築時の追加順 (`graph!` の場合はソース記述順) を保持する」ことを
  実装詳細から仕様に昇格させた。生成アクセサ (`{label}`/`{label}_ids`/
  `{label}_pairs`) の doc コメントに明記し、README に「`(0..*)` エッジの
  順序保証」節を追加。`orgchart_macro.rs` に順序を固定するテストを追加した
  (始点キーをまたぐ列挙順まではこの保証の対象外であることも明記)。
