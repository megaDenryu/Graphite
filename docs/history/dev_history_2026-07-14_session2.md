...
(引用中の綴りは当時のもの。現在は `docs/history/dev_history_2026-07-14_session1.md` にある)
...
## 3.13 種の消化 — スプライス・複数形廃止・キーの意味論 v4.2・ComputeGraph (2026-07-18)

完了札の本文に将来の作業を埋めていた運用をユーザーに指摘され
(「完了した札の中に何かを残してるの・・？」)、種を独立札 #13〜#16 に抽出。
続けて「それを進めよう」の指示で全てを消化した。

- **#16 複数形構文の廃止**: `node 型名(複数形);` を削除 (ストレージ名が内部専用に
  なり明示の意義が消えていたため)
- **#13 スプライス構文 + extend 統一** (`docs/graph_splice.md`): graph! の項に
  `..式` (実行時コレクションの合成)。脱糖先の必要から一括挿入を単一の総称
  `extend` に統一し、直前に入れた extend_nodes/extend_edges は痕跡なしで置換。
  キーの二重体系は「キー = 名前 (出所がソースコード上かデータ上かの違いだけ)」
  と整理
- **#15 キーの意味論 v4.2** (`docs/node_id_v4_2.md`) — 本節の白眉。
  オーケストレータが「実需要が出たら再訪」と需要駆動で先送りしようとしたところ、
  ユーザーの指摘で枠組み自体が誤りと判明:

  > 今てなに？意味不明な問ですね。言語ですよこれ？

  言語は永続する成果物であり、問いは「いつ要るか」ではなく「意味論として何が
  正しいか」。意味論の軸で問い直した結果、ユーザーが**「キーは個体の名前」**
  (グラフ = 同じ個体宇宙についての主張の集合) を選択。規則
  **「型を宣言した者が、その Id も宣言する」**が導かれ、ノード Id はユーザー宣言
  (`PersonId` の 1 行は「この型は名前で識別される個体」という意味のある宣言)、
  辺 Id は schema 生成のまま、schema 間のキー共有は衝突ではなく当然の帰結になった。
  移行時の副産物: 無向辺の unique pair 正規化が暗黙に Ord を要求していたバグを
  発見・修正 (必要 derive を仕様どおり最小に)。残る実制約は「2 つの
  {Schema}Node trait が同一スコープにあるときのメソッド曖昧性」(README に記載)
- **#14 ComputeGraph** (`docs/compute_graph.md`): flow! (即時実行の脱糖) の対になる
  「計算グラフを実行時の値として持つ」ランタイムエンジン。pull 型の遅延評価 +
  差分再計算 (dirty 祖先だけをトポロジカル順に各 1 回 = glitch-free)、循環は
  freeze 時に CycleError。関数は Box&lt;dyn Fn&gt; (原則 5 は生成コードの規律であり、
  ランタイムエンジンの dyn は Rust の正道 — 線引きを rustdoc に明記)。
  reactive-cells の Engine の一般化で、freeze の検証は汎用 Graph の機構を再利用

運用の教訓 2 件をメモリに固定: (1) 完了札に種を埋めない (閉じる前に独立札へ抽出)、
(2) 言語の意味論を需要駆動で先送りしない。

## 4. 現在の状態

- テスト (実測、種4件 [#13 graph!スプライス構文+extend統一・#16 node宣言複数形
  明示構文廃止・#15 ノードIdユーザー宣言化v4.2・#14 ComputeGraph] 完了時点):
  コア124 (`crates/graphite` 単体40 + 統合79
  [`compile_fail`1・`f64_attrs`1・`flow`10・`graph_cross_module`1・
  `graph_splice`5 (新規)・`keyed_table_insertion_order`2・
  `node_id_shared_across_schemas`1 (新規)・`orgchart_handwritten`8・
  `orgchart_macro`31・`orphan_node_no_warning`1・`sources_of`9・
  `undirected_edges`9] + `graphite-macros`単体2 + doctest3) + examples 7本
  (hello-graph 16 / build-pipeline 32 / org-analyzer 11 / dialogue-engine 14 /
  state-machine 15 / async-dag 15 / reactive-cells 25)、合計 252、全通過。
  `crates/graphite` 単体は ComputeGraph の完了条件テスト追加で31→40に増加。
  統合テストは複数形明示構文の廃止に伴い `explicit_plural_field.rs` が
  削除される一方、`graph_splice.rs` (新規5件)・
  `node_id_shared_across_schemas.rs` (新規1件) が追加され74→79に、doctest は
  compute モジュールの例追加で2→3に増加。examples 7本の内訳は前回実測から
  変化なし (v4.2 ノードId宣言化・ComputeGraph 導入後も既存テストは全通過)。
  構文は v4/v4.1/v4.2 のみ (v1〜v3は検出・移行診断なしで完全廃止。v4.2で
  ノードIdの生成も廃止しユーザー宣言参照に一本化)
- コミット (v3→v4移行分、古→新): `342ee0b` (v4設計文書) → `3fff112`
  ...
  (docs: hello-graphにflow!教材節追加・READMEチートシート追記) → `2de6555`
  (docs: READMEにキーの設計・一括構築・逆引きsources_ofを明文化) → `3da0b94`
  (docs: 開発履歴§4をWave1〜3完了時点の実測値へ更新) → `05499f4`
  (docs: graph! スプライス構文とextend統一の設計方針を追加 (#13/#16)) →
  `57773fb` (schema_dsl,schema_codegen: node宣言の複数形明示構文を廃止
  (#16)) → `aa3b55c` (graph!: extendを単一の総称メソッドへ統一し、
  スプライス構文(..式)を追加 (#13/#16)) → `e372bf4`
  (tests: graph!スプライス構文の統合テストとtrybuildケースを追加 (#13)) →
  `cf232d7` (docs: ノードId宣言のユーザー化 (v4.2) の一次資料を追加) →
  `7e6dcee` (feat!: ノードIdをgraph_schema!の生成対象から外しユーザー宣言
  参照に変更 (v4.2)) → `8fa4bb9`
  (examples: 7本をノードId宣言化 (v4.2) へ移行しREADME/modeling_guideを更新)
  → `f8f3da3`
  (docs: ComputeGraph (遅延実行・差分再計算エンジン) の設計を追加
  (Fudaba #14)) → `c154343`
  (feat: ComputeGraph (遅延実行・差分再計算エンジン) を実装 (Fudaba #14)) →
  `0162472`
  (test: ComputeGraphの完了条件テストを追加、reactive-cells READMEに
  汎用版への参照を追記) → (本節更新の docs コミットが続く)
- リモート: 前回の push (Wave1〜3, `824429d`〜`1542b6a`) 時点で
  `origin/main` に追随済みだったが、その後 `05499f4`〜`0162472` の
  10コミット (種4件: #13 graph!スプライス構文+extend統一・#16 node宣言複数形
  明示構文廃止・#15 ノードIdユーザー宣言化v4.2・#14 ComputeGraph) が積み上がり
  再び `origin/main` より先行。本節更新コミットとまとめて `origin/main` へ
  push済み
