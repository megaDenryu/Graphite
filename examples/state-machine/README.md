...
`OrderState -> OrderState` の自己ループ的な辺種別として宣言している。
...
これがそのまま「設定ファイル」に相当する一枚絵になっている。v4 では辺も
第一級のキー付き要素なので、全ての辺行に一意な辺キー (端点+イベント名から
読める名前、例: `submit_draft` = `draft` からの `Submit`) を付ける。読み方:

- 正常フローは `Submit(draft -> pending_payment)`、`Pay(pending_payment ->
  paid)`、`Ship(paid -> shipped)`、`Deliver(shipped -> delivered)` という
  一直線。
...
`validate::validate` は6種のイベント辺 (`Submit`/`Pay`/`Ship`/`Deliver`/
`Cancel`/`Refund`) を全部束ねて、ラベルの区別を捨てた汎用
