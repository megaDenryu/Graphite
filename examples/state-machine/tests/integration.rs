...
fn payのiterは各始点キーにつき1本ずつしか無い_多重度01の保証() {
    ...
    for (from, _to) in g.pay().iter() {
        assert!(
            seen_sources.insert(from.clone()),
            "同じ始点キー {from:?} から2本以上のpay辺があってはならない (多重度0..1違反)"
        );
    }
}
...
fn 終端状態集合に含まれる状態は正規fsmでは出て行く辺を持たない() {
    ...
    for terminal in fsm::terminal_states() {
        assert!(g.submit().id_of(&terminal).is_none());
        assert!(g.pay().id_of(&terminal).is_none());
        assert!(g.ship().id_of(&terminal).is_none());
        assert!(g.deliver().id_of(&terminal).is_none());
        assert!(g.cancel().id_of(&terminal).is_none());
        assert!(g.refund().id_of(&terminal).is_none());
    }
}
