...
統合) でマクロは `graphite::static_graph_schema!` として Graphite 本体crate群
...
- `src/main.rs` — TS の union type で書いた schema 表現
  ...
  あちらは `dynamic_graph_schema!` が生成すべきコードの形を確かめるための手書き)
- `src/bin/static_graph.rs` — 全個体がコンパイル時に確定する静的グラフの手書き
  到達点 (issue #24)。仕組み (`辺`・`結ぶ`・`ノードタグ`・`台帳`・`ノード参照`・
  `辺参照`) は `src/lib.rs`/`src/仕組み/` へ切り出し済み。`static_graph_schema!` が
