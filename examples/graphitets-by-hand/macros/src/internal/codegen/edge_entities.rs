// 生成物3: 辺達。辺の実体の唯一の所有者。フィールド型は `{種別}の辺<'a>`
// (生成物1) を直に書く。フィールド名 (役割名) はschema側の宣言から取る
// (instance側は個体名・積み荷式しか持たない)。
...
fn 配線を生成する(辺: &具体辺宣言, 型宣言: &crate::schema::input::辺宣言) -> TokenStream {
    let 名前 = &辺.名前;
    let 型名 = format_ident!("{}の辺", 辺.種別);
    let 積み荷フィールド = 積み荷フィールドを生成する(型宣言, 辺);
    match (&辺.形状, &型宣言.形状) {
        (具体形状::有向 { 始点, 終点, .. }, 型形状::有向 { 始点役割, 終点役割, .. }) => quote! {
            #名前: #型名 { #始点役割: &ノード達.#始点, #終点役割: &ノード達.#終点, #積み荷フィールド }
        },
        (具体形状::無向 { 端点1, 端点2, .. }, 型形状::無向 { .. }) => quote! {
            #名前: #型名 { 端点1: &ノード達.#端点1, 端点2: &ノード達.#端点2, #積み荷フィールド }
        },
        _ => unreachable!("相互検証済みなので向きは一致している"),
    }
}

fn 積み荷フィールドを生成する(型宣言: &crate::schema::input::辺宣言, 辺: &具体辺宣言) -> TokenStream {
