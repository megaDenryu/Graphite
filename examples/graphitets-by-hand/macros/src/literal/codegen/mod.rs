...
//! 辺達 → 型別名 → 所属辺メソッド → 参照の層とグラフ本体」で、手書き到達点
//! (`static_graph.rs`) の層順 (仕組み→実体の層→参照の層) と揃えてある。
//! 各生成物の中身は配下の module が持ち、この module 本体は並び順だけを知る。
...
pub fn 生成する(入力: &静的グラフ入力) -> TokenStream {
    let 実体型索引: HashMap<String, proc_macro2::Ident> = 入力
        .ノード宣言達
        .iter()
        .map(|ノード| (ノード.名前.to_string(), ノード.実体型.clone()))
        .collect();
