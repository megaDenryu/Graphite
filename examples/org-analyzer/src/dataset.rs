...
//! ある。この流れの分割は、社員一覧・部署割当・4種の辺の下書きという生成途中の
//! 中間状態を外へ晒す。そのため書き手は語彙表 (`name_pool`)・擬似乱数
//! (`lcg`)・分布 (`distribution`)・キーの綴り (`element_id`)・結果の型
//! (`generated_org`)・異常の注入 (`anomaly_injection`) を分けたうえで、生成の
//! 流れ自体を1つに統合している。
//!
//! このファイルは1ファイル100行の原則の例外である (区分: 統合による超過)。
//! このファイルは1つのシードから1つの組織データを合成しきる1本の流れを持つ。
//! このファイルの分割は、生成途中の中間状態を外へ出す。超過を許す根拠の台帳
