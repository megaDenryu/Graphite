...
// 生成元: examples/org-analyzer/src/schema.rs:69
...
pub(super) const __GRAPHITE_SCHEMA_FINGERPRINT: [u64; 4] = [
    10120336357431590725u64, 7458874168771904500u64, 15004906976925199807u64,
    8951862394347320987u64,
];
...
pub enum Violation {
    ...
    /// このエッジが未知の始点キーを参照している。
    AssignedUnknownSource { edge: AssignedId, source: EmployeeId },
    /// このエッジが未知の終点キーを参照している。
    AssignedUnknownTarget { edge: AssignedId, target: ProjectId },
    /// このエッジ種別のキーが重複している。
    SponsorsDuplicateKey(SponsorsId),
    /// このエッジが未知の始点キーを参照している。
    SponsorsUnknownSource { edge: SponsorsId, source: DepartmentId },
    /// このエッジが未知の終点キーを参照している。
    SponsorsUnknownTarget { edge: SponsorsId, target: ProjectId },
    /// このエッジ種別の `each` 制約違反 (出次数)。
    SponsorsDepartmentEachViolation { source: DepartmentId, count: usize },
}
...
impl Builder {
    ...
    fn freeze_collecting(self) -> Result<Graph, Vec<Violation>> {
        ...
        for position in __graphite_node_department.positions() {
            ...
            let key = __graphite_node_department
                .get_at(position)
                .expect("列挙した内部位置はノード表に存在する")
                .0;
