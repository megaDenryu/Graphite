...
`graphite::static_graph_schema!` (issue #24。全個体がコンパイル時に確定するグラフ)
の正式な利用例です。`dynamic_graph_schema!`/`graph!` (実行時に個体を追加でき、
...
| ファイル | 内容 |
|---|---|
| `src/domain.rs` | `static_graph_schema!` の外で宣言する実体型 (社員・部署・任命記録・経緯記録) |
| `src/main.rs` | schema宣言 (`static_graph_schema!`)・instance宣言 (`組織!`)・構築から辿りまでの実演。コンパイルエラーの実測コメント (存在しない辿り・多重度違反・対一意違反) を含む |
...
`src/main.rs` の schema宣言 (`static_graph_schema! { schema 組織 { .. } }`) と
instance宣言 (`組織! { graph 開発チーム; .. }`) の対を読むと、DSLの形が
一目で分かります。`組織!` は `static_graph_schema!` が展開時に生成する
