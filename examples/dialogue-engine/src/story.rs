//! 本編シナリオ「月面基地アルテミスIII、通信途絶」の `graph!` リテラル。
//!
//! 1本の `graph!` 呼び出しが30シーン・4エンディング・56本の選択肢・4本の
//! finale を宣言する。このファイルが担当するのは「本編シナリオという1つの
//! データ」だけであり、検証テスト用の壊れたシナリオは `broken_story.rs` に
//! ある。
//!
//! このファイルは1ファイル100行の原則の例外である (区分: 宣言的データリテラ
//! ル)。このファイルは1本の `graph!` 呼び出しを持つ。この呼び出しは30シーン・
//! 4エンディング・56本の選択肢を宣言する1つのデータである。超過を許す根拠の
//! 台帳は `docs/development/line_count_ledger.md` にある。
//!
//! 分けられない理由: シーンを外へ出す手段としてスプライス構文 `..式`
//! (`docs/graph_splice.md` §1) があるが、スプライスの要素は実行時の
//! `(String, 値)` の対であり、静的な項が作る識別子の束縛を持たない。矢印記法
//! `Choice(start -[ChoiceEdge { .. }]-> arrival)` の両端はその識別子を指すため、
//! シーンをスプライスへ移すと56本の選択肢が全て `Choice::new(SceneId(..), ..)`
//! の呼び出しへ書き換わる。それはこの example が見せている当のもの (README
//! 「矢印記法は 30 シーン・56 選択肢の規模でも十分読める」) を消す変更になる。
//!
