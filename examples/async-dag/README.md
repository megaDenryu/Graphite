...
| 依存順序が守られているかの検証 | 手作業でログを目で確認 (or 検証コード無し) | 実行ログの `start`/`end` を `g.depends_on_iter()` で全数検査可能 (`tests/execution_order.rs`) |
...
検証しているのが `tests/wave_composition.rs` である (波の**内容**は
