...
//
// `dependency_graph` は `Feeds`/`Lhs`/`Rhs` エッジを1つに射影した汎用グラフ
// である。`reachable_from`/`topological_sort` はここに1回だけ委譲する
// (`graphite::Graph` が既に持つ水準1アルゴリズムを再実装しない)。
// `topo_order` は構築時に1回だけ計算したトポロジカル順序である。依存構造
// (`dependency_graph`) は構築後不変なので、この順序も更新ごとに再計算する
// 必要はない。
pub struct Engine {
    graph: Sheet::Graph,
