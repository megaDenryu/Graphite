...
判定は行頭だけを見て、行の途中の状態までは追わない。この数え方には2つの限界があり、
どちらも実際より少なく数える側へ働く。1つ目は文字列リテラルの中身を解析しないこと
(複数行の生文字列リテラルの中に行頭が // の行を書くと、その行を数えない)、2つ目は
ブロックコメントの入れ子を追わないこと (最初の */ で閉じたとみなす) である。
