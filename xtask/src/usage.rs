...
//! このファイルは `cargo xtask` の各コマンドの使い方と、
//! 検査の範囲・限界の説明文という1つのデータを持つ。この説明文の分割は、
//! 1つの説明を複数ファイルへ散らす。超過を許す根拠の台帳は
...
ファイル、台帳に無いのに冒頭コメントが定型の固定部分「このファイルは1ファイル100行の
原則の例外である」を書いて例外を名乗っているファイル、読み込みに失敗したファイル、
台帳の行として読めない表の行である。
...
コメントの開始とみなさない。この数え方には3つの限界があり、走査が実際より少なく数える
向きと多く数える向きの両方が現れる。
1つ目は走査が生文字列 (r\"...\" ・ r#\"...\"#) を通常の文字列リテラルとして扱うことで
あり、この扱いは逆向きの2つの誤りを生む (走査は中の \" でリテラルが閉じたとみなすため、
走査は実際より少なく数える。逆に、生文字列が \\ で終わる形 (Windows のパス等) では、
走査は \" を打ち消しとみなして読み進み、以降の /* を見落とすため、走査は実際より多く
数える)。
