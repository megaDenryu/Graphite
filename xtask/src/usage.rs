...
実際のコード行数の両方である。検査する違反は6種類ある。台帳に無いのに100行を
超えているファイル、150行の上限を超えているファイル、台帳にあるのに100行以内へ
収まったファイル (台帳から削除すること)、台帳にありながら検査対象に実在しない
綴り、読み込みに失敗したファイル、台帳の行として読めない表の行である。報告には
数えたファイル数・台帳の件数・再設計待ちの件数を出す。

コード行の数え方と区分の定義は docs/development/line_count_ledger.md が正本である。
数えないのは空行とコメントだけの行であり、#[cfg(test)] を付けたテストモジュールの
中は数える。数える処理は syn の構文解析ではなく行の走査で行う。コンパイルが通らない
ことを目的にしたファイル (crates/graphite/tests/ui 配下) も対象に含めるためである。
...
150行の上限は、区分「宣言的データリテラル」には適用しない (規約の別枠であり、
行数で切ると宣言の一覧性が壊れるため)。区分「再設計待ち」は当面の猶予として違反に
せず、件数だけを報告する。issue #28 のやること4 がこの区分の全件を再設計する。
