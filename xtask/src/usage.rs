...
実際のコード行数の両方である。check-line-counts は6種類の違反を報告する。台帳に
無いのに100行を超えているファイル、150行の上限を超えているファイル、台帳にあるのに
100行以内へ収まったファイル (台帳から削除すること)、台帳にありながら検査対象に実在
しない綴り、読み込みに失敗したファイル、台帳の行として読めない表の行である。
...
持ち回る。この数え方には2つの限界があり、2つは働く方向が違う。1つ目は走査が文字列
リテラルの中身を解析しないことであり (走査はリテラルの中の // と /* をコメントの開始と
みなすため、走査は実際より少なく数える)、2つ目は走査がブロックコメントの入れ子を
追わないことである (走査は最初の */ でコメントを閉じたとみなし、外側のコメントの残りを
コードとして数えるため、走査は実際より多く数える)。
