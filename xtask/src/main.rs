...
行範囲」に実在することを見る。2つ目は引用の鮮度であり、引用の「全行」が参照先の
「ファイル全体」に実在することを見る。前者は引用がどの範囲から取られたかを見て、
後者は引用が現在のコードから取られたかを見る。先頭3行の上限は前者だけのもので
...
`text` のフェンスは出典を持つ引用ではないため、この総数に数えない)。検査が届かな
かった範囲を件数で見えるようにするために報告する。

文書の側は、引用をコードから改変せずに写す必要がある。鮮度の判定が見るのは「その行が
参照先ファイルに実在するか」であり、変数名を短くする・型引数を省くといった改変をすると
違反になる。`docs/desugaring_reference.md` の冒頭が宣言しているとおり、原文との違いは
4つ (先頭の字下げの除去・doc コメント記号の除去・「(署名のみ抜粋)」と記した箇所での
本体省略とシグネチャの1行化・出典範囲内の一部メソッドの無印省略) に限る。
